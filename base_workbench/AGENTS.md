...
Po przejściu testów zadania uruchom osobno lokalny fast-check z root. Pełna regresja nie należy do zwykłego autora. Droga dostarczenia jest zamknięta: `implementacja -> lokalne testy zadania -> lokalny fast-check -> commit -> tools/publish_agent_pr.ps1 (push + PR) -> KONIEC`. Brak commitu albo opublikowanego PR oznacza zadanie niedostarczone. Nie pushuj do `main`, nie polluj i nie babysituj kolejki.
