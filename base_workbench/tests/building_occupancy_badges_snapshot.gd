...
func _ready() -> void:
	# Native cursor placement must not pre-hover a slot before the harness emits
	# its deterministic mouse_entered/mouse_exited sequence.
...
func _save_snapshot_at_size(file_name: String, target_size: Vector2i) -> bool:
	...
	if not DirAccess.dir_exists_absolute(output_directory):
		...
		if directory_error != OK:
			push_error("Could not create the isolated occupancy snapshot directory. Error: %d" % directory_error)
			get_tree().quit(1)
			return false
