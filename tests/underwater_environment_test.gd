...
func _run() -> void:
	...
	renderer.rock_detail_texture = ResourceLoader.load("res://assets/diving/world/materials/underwater_rock_detail_tile_v1.png")
	...
	_assert(str(renderer_state.get("graphics_quality", "")) == "low", "Jakość grafiki musi dotrzeć do materiału terenu.")
	_assert(bool(renderer_state.get("reduced_motion", false)), "Reduced motion musi dotrzeć do materiału terenu.")
	_assert(is_equal_approx(float(renderer_state.get("anim_time", -1.0)), 3.25), "Renderer terenu musi korzystać z jawnego czasu animacji.")
	_assert(int(renderer_state.get("active_chunk_count", 0)) == 3, "Renderer ma deduplikować i odrzucać chunki poza mapą.")
	_assert(bool(renderer_state.get("uses_global_depth_profiles", false)), "Wszystkie chunki muszą dzielić globalny profil głębokości bez pasów na granicach.")
	_assert(int(renderer_state.get("water_material_count", 0)) == 1, "Chunki w różnych regionach powinny współdzielić jeden materiał wody.")
	_assert(int(renderer_state.get("terrain_material_count", 0)) == 1, "Chunki w różnych regionach powinny współdzielić jeden materiał skał.")
	_assert(bool(renderer_state.get("uses_global_water_layer", false)), "Woda powinna być pojedynczą warstwą świata bez szwów geometrii chunków.")
	_assert(bool(renderer_state.get("uses_global_terrain_layer", false)), "Proceduralna skała powinna być pojedynczą warstwą świata bez szwów geometrii chunków.")
	_assert(bool(renderer_state.get("uses_derived_contour_sdf", false)), "Krawędź skał powinna korzystać z prezentacyjnego SDF wyprowadzonego z maski semantycznej.")
	_assert(renderer.contour_sdf != null and renderer.contour_sdf.get_size() == renderer.contour_mask.get_size(), "SDF i kanoniczna maska muszą mieć identyczną siatkę źródłową.")
	...
	if sdf_manifest_variant is Dictionary:
		...
		_assert(str(sdf_manifest.get("source_sha256", "")) == FileAccess.get_sha256("res://assets/diving/world/map_v2/world_collision_grid.png").to_lower(), "Manifest SDF musi wskazywać bieżący hash kanonicznej maski.")
	...
	_assert(terrain_shader != null and terrain_shader.code.contains("terrain_fbm(world_position / 610.0") and terrain_shader.code.contains("terrain_fbm(world_position / 360.0") and terrain_shader.code.contains("terrain_fbm(world_position / 188.0"), "Makrorelief, osad i minerały muszą korzystać z wielooktawowego pola świata.")
	_assert(terrain_shader != null and terrain_shader.code.contains("if (!has_detail_texture)") and terrain_shader.code.contains("return dot(texture(rock_detail_texture, detail_uv).rgb"), "Produkcyjny kolor i relief skały muszą korzystać z autorskiego kafla, a proceduralna siatka wyłącznie z fallbacku.")
	var rock_detail_import := FileAccess.get_file_as_string("res://assets/diving/world/materials/underwater_rock_detail_tile_v1.png.import")
	...
	var detail_manifest_variant = JSON.parse_string(FileAccess.get_file_as_string("res://assets/diving/world/materials/underwater_rock_detail_tile_v1.json"))
	...
	if detail_manifest_variant is Dictionary:
		...
		_assert(str(detail_manifest.get("source_sha256", "")) == FileAccess.get_sha256("res://assets/diving/world/materials/underwater_rock_detail_v1.png").to_lower(), "Manifest detalu musi wskazywać bieżący asset autorski.")
		_assert(str(detail_manifest.get("output_sha256", "")) == FileAccess.get_sha256("res://assets/diving/world/materials/underwater_rock_detail_tile_v1.png").to_lower(), "Manifest detalu musi wskazywać rzeczywisty bezszwowy kafel.")
