...
func _initialize() -> void:
	...
	encounter.configure("fixture_rescue", definition)
	...
	_assert(result.rescue_outcomes.get("fixture_rescue", {}).get("status", "") == "rescued", "A safe return should carry a stable persistent outcome.")
	...
	_assert(world_only_state.underwater_world.rescued_or_dead_survivors.has("fixture_rescue"), "The rescue outcome should persist in WorldDelta.")
	...
	critical_encounter.configure("fixture_rescue", definition)
	...
	_assert(death_result.rescued_survivors.is_empty() and death_result.rescue_outcomes.get("fixture_rescue", {}).get("status", "") == "dead", "Losing a dive while towing should never add the survivor to the base.")
