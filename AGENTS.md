...
- Każdy zapisujący agent pracuje w osobnym pełnym Git worktree utworzonym z aktualnego `origin/main` i na gałęzi `codex/<owner>/<task-slug>`. Osobny CWD w tym samym checkoutcie nie izoluje indeksu Git, plików ani Godota.
- Codzienny bootstrap to `tools/setup_agent_worktree.ps1 -FromOriginMain ... -Create`. Nie wymaga candidate/run receiptów ani lokalnego LKG, ale wymaga identity task/thread/owner, briefu i zamkniętego write-setu. Helper jako ostatni marker tworzy assignment `WAITING_ACK`; agent przechodzi do zapisu dopiero po zgodnym ACK i stanie `RUNNING`.
- Przed commitem agent uruchamia bramkę EOL, testy celowane i `validate --assignment ... --diff`. Nie czyta ani nie modyfikuje live worktree innego autora; zależność pobiera po `fetch` z exact SHA albo PR.
- Po zielonych testach agent tworzy logiczny commit i uruchamia `tools/publish_agent_pr.ps1`. Helper wymaga świeżej bazy `origin/main`, czystego worktree, dokładnie jednego zgodnego assignmentu `RUNNING` oraz assignment-scoped owner/write-set diffu. Pushuje tylko exact HEAD bieżącej gałęzi, otwiera zwykły PR, wysyła zaufany dispatch weryfikacyjny i włącza natywne auto-merge `--squash --match-head-commit`; nie czeka na ciężkie testy po merge.
- Kandydacki workflow bez sekretów daje szybki feedback. Wymagany `fast-green` publikuje zaufana GitHub App dopiero z default-branch receivera po ponownej walidacji exact base/head/diff/ownera/EOL. Zmiana wrażliwa, w tym każda zmiana control-plane'u, przechodzi tam automatyczny, strukturalny Codex gate bez wykonywania kodu kandydata w jobie z sekretem. Nie ma ręcznego review, merge, environment approval ani bypassu.
- Zielony PR trafia do natywnej merge queue, która ponawia App-owned check na syntetycznym `merge_group`, i jest scalany jako squash. Czerwony wynik pozostawia PR do poprawki nowym commitem. Cel szybkiej informacji zwrotnej to 30-60 sekund; pełna regresja nie należy do premerge.
- Pełna regresja uruchamia się po pushu do `main` w tle: niedestrukcyjny check Mapy, cztery odseparowane shardy headless i jeden natywny. Nowszy `main` anuluje starszy przebieg. Porażka daje czerwony raport i artefakty, ale nie cofa `main`, nie blokuje kolejnych PR-ów i nie zatrzymuje agentów.
- Osobny standalone source clone i lokalny builder są zarządzane przez `tools/sync_play_main.ps1`. Source clone jest tylko czystym mirrorem `main`; runner utrzymuje trwałą kolejkę wszystkich SHA, sprawdza je sekwencyjnie w osobnych immutable katalogach i dopiero po LFS fsck, eksporcie, pełnych testach oraz smoke EXE atomowo przełącza `current`. FAIL zapisuje czerwony status i alert bez zmiany ostatniego grywalnego `current`; brak presetów eksportu kończy się `EXPORT_BOOTSTRAP_REQUIRED`.
- `underwater_map_workbench/map_manifest.json`, `UnderwaterMap.tscn`, mapowe metadane i `structures/*/generated/**` pozostają wspólnym zestawem Mapy. Prywatny seal/build/check nie zdobywa globalnej blokady; tylko krótka publikacja jednego integratora używa `map-promotion`, rehashu i per-path CAS/rollback.
- Runner wykonuje testy na zamkniętym snapshotcie, izoluje `.godot`, `user://`, TEMP/TMP, logi, porty i capture oraz odrzuca `-InPlace`. Równoległy Godot jest dozwolony wyłącznie w osobnych pełnych workspace.
