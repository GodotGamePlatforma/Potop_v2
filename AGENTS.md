...
- Root-routed zadanie seal+pin ma zamknięty write-set obu właścicieli i uruchamia pełny build/check oraz testy pakietu i Mapy. `--seal-structure-package` nadal zapisuje tylko lokalny manifest, a `--refresh-structure-package` nigdy nie przepisuje prywatnych źródeł; atomowość dotyczy wspólnego branch/PR. Jeżeli konkurencyjny PR zmieni `main` wcześniej, późniejszy autor aktualizuje bazę do bieżącego `origin/main` i ponownie uruchamia seal, refresh, builder oraz cały wymagany zestaw testów.
...
- Każde zadanie ma osobny pełny Git worktree i gałąź `codex/<owner>/<task-slug>` utworzoną z aktualnego `origin/main`; zadanie seal+pin struktury używa `codex/structure-<id>/<task-slug>`. Sam osobny CWD we wspólnym checkoutcie nie daje izolacji.
- Agent wdraża zmianę w swojej allowliście, uruchamia proporcjonalne testy lokalne, sprawdza pełny diff i tworzy jeden logiczny PR dla jednego zadania. Nie pushuje bezpośrednio do `main` i nie uzgadnia kolizji przez ostatni zapis.
- `fast-check` jest obowiązkową bramką PR. Tylko jego jawny `PASS` pozwala dodać PR do merge queue; każdy inny wynik blokuje enqueue i merge, również dla rzadkiej zmiany control-plane. Pełny `integration-green` działa wyłącznie na merge group `aktualny main + dany PR`; dopiero jego `PASS` pozwala na squash do `main`.
...
- Docelowo workflowy, narzędzia CI, konfigurację `integration-green` i control-plane buildera chronią reguły GitHub. Zwykły PR nie może ich zmieniać; rzadka zmiana wymaga osobnego PR i ręcznego zatwierdzenia właściciela, które nie omija `fast-check PASS` ani `integration-green PASS` merge group.
...
- Assignmenty, ACK, lokalne LKG, receipty, FROZEN hand-offy, osobni agenci-audytorzy i rozmowy między agentami nie należą do procesu. Do czasu wdrożenia merge queue, pełnej ochrony ścieżek i finalnego buildera agent nadal kończy pracę na własnym PR oraz lokalnych testach; nie odtwarza usuniętych bramek ręcznie. Bieżącą lukę implementacyjną opisuje `.ai/PROJECT_CONTEXT.md`.
...
- Zmiana `structures/<id>/` wymagająca nowego seala jest jednym root-routed zadaniem. W tym samym worktree i PR wykonaj `--seal-structure-package <id>`, `--build-structure <id>`, `--check-structure <id>`, oba testy pakietu, `--refresh-structure-package <id>` dla dokładnego SHA-256 manifestu, pełny mapowy `--build`/`--check` oraz pełne testy Mapy. Po aktualizacji bazy z konkurencyjnego `main` cały builder i wymagane testy uruchom ponownie.
