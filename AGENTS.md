...
- Agent skupia się na zadaniu: `implementacja -> lokalne testy zadania -> lokalny fast-check`. Porażkę poprawia i powtarza właściwy etap. Po `PASS` jedno `tools/finish_agent_task.ps1` wykonuje logiczny commit i deleguje exact push + PR do `publish_agent_pr.ps1`; dla zwykłej zmiany włącza squash auto-merge. Agent nie pushuje do `main`, nie polluje i nie babysituje kolejki. Nie mówi „gotowe” ani „wdrożone”, dopóki publisher nie potwierdzi `LocalHead = RemoteHead = PullRequestHead` dla dokładnie jednego otwartego PR.
