...
Jeżeli główny zakres zadania dotyczy konkretnej mapy podwodnej lub grafiki nurkowania, niezależnie od początkowego katalogu roboczego najpierw przeczytaj `underwater_map_workbench/AGENTS.md`, ustaw katalog roboczy narzędzi na `underwater_map_workbench/` i zastosuj jego routing. Warsztat jest właścicielem całego aktywnego pakietu mapy i potrzebnych grafik podwodnego gameplayu; root zachowuje ogólne mechaniki, dane domenowe i integrację Godot. Jest to jedyny zatwierdzony wyjątek od poniższego protokołu pełnego odczytu i nie osłabia globalnej bramki rozbieżności, ochrony zapisu ani zasad testów.
...
`underwater_map_workbench/` jest jedynym zatwierdzonym wyjątkiem domenowym. Posiada aktywny manifest, generowaną scenę, pojedynczy builder, pojedynczy smoke test, shadery środowiska oraz potrzebne grafiki podwodnego gameplayu w `assets/gameplay/`. Jego `.ai/PROJECT_CONTEXT.md` opisuje bieżącą migawkę pakietu, `.ai/DECISIONS.md` trwałe decyzje mapy, `README.md` onboarding i komendy, a lokalny `AGENTS.md` proces wykonawczy. Nie wolno kopiować tam globalnych reguł gry, właścicieli stanu ani persistence; root nie może natomiast utrzymywać drugiej specyfikacji, sceny, manifestu lub katalogu grafik nurkowania. Korzeniowe dokumenty zachowują przekrojowy kontrakt i konsekwencje dla całego projektu zgodnie z ARD-0102.
