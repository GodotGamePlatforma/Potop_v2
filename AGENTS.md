...
- Baza, jej budynki, lokalne zarządzanie, scena, UI, grafika i dane definicyjne: `base_workbench/AGENTS.md` i CWD `base_workbench/`.
...
Jeżeli głównym zakresem zadania jest `BaseScene`, lokalne środowisko Przystani, sześć budynków, ich definicje i dane, gospodarka Bazy, obsada, racje, opieka, wydarzenia osady, warsztat, lokalny interfejs, grafika, audio albo test/capture Bazy, przejdź do `base_workbench/AGENTS.md`. Warsztat Bazy jest jedynym źródłem tych elementów i działa w projekcie nadrzędnym; nie twórz drugiego `project.godot`, kopii sceny w `scenes/base`, kopii systemów pod `scripts/base` ani drugiego katalogu assetów lub danych Bazy w root.
...
- Agent skupia się na zadaniu: `implementacja -> lokalne testy zadania -> lokalny fast-check`. Porażkę poprawia i powtarza właściwy etap. Dopiero po `PASS` wykonuje `commit -> tools/publish_agent_pr.ps1 (push + PR) -> KONIEC`. Brak commitu albo opublikowanego PR oznacza zadanie niedostarczone, a nie ukończone. Lokalne testy zadania i fast-check są osobnymi krokami; fast-check może defensywnie powtórzyć część testów, ale ich nie zastępuje. Agent nie pushuje do `main`, nie polluje i nie babysituje kolejki.
