...
- Chronione workflowy, narzędzia CI, konfiguracja `integration-green` i control-plane buildera są egzekwowane przez reguły GitHub. Zwykły PR ich nie zmienia; rzadka zmiana wymaga ręcznego zatwierdzenia właściciela.
...
- Assignmenty, ACK, lokalne LKG, receipty, FROZEN hand-offy, osobni agenci-audytorzy i rozmowy między agentami nie należą do procesu. Do czasu wdrożenia merge queue i finalnego buildera agent nadal kończy pracę na własnym PR oraz lokalnych testach; nie odtwarza usuniętych bramek ręcznie. Bieżącą lukę implementacyjną opisuje `.ai/PROJECT_CONTEXT.md`.
