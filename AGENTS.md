...
- Baza, jej budynki, lokalne zarządzanie, scena, UI, grafika i dane definicyjne: `base_workbench/AGENTS.md` i CWD `base_workbench/` po scaleniu atomowej migracji warsztatu.
- Systemy ogólne, kampania, misje, pogoda, difficulty, ocaleńcy, nurkowanie, zapis albo integracja kilku właścicieli: ten plik i CWD root.
...
Jeżeli głównym zakresem zadania jest `BaseScene`, lokalne środowisko Przystani, sześć budynków, ich definicje i dane, gospodarka Bazy, obsada, racje, opieka, wydarzenia osady, warsztat, lokalny interfejs, grafika, audio albo test/capture Bazy, po atomowym wydzieleniu przejdź do `base_workbench/AGENTS.md`. Warsztat Bazy jest wtedy jedynym źródłem tych elementów i działa w projekcie nadrzędnym; nie twórz drugiego `project.godot`, kopii sceny w `scenes/base`, kopii systemów pod `scripts/base` ani drugiego katalogu assetów lub danych Bazy w root.

`GameState`, trwałe stany, `SaveManager`, walidacja i migracje, atomowy koniec dnia, kampania, misje, difficulty, pogoda, rozwój i choroby ocaleńców, ogólne systemy nurkowania oraz integracja pozostają w root. Warsztat korzysta z nich jako z publicznych zależności projektu nadrzędnego. Zmiana reguły widocznej dla gracza, semantyki stanu, kolejności rozliczenia, persistence albo publicznej granicy Root–Baza jest zadaniem integracyjnym prowadzonym z root.

Do chwili obecności kompletnego `base_workbench/AGENTS.md` oraz atomowej relokacji źródeł istniejące pliki Bazy pozostają fizycznie w Root, a samo wydzielenie prowadzi owner `integration`. Po migracji zwykłe zadania Bazy używają ownera `base` i zapisują wyłącznie `base_workbench/**`.
...
|---|---|---|---|
| Root: kampania, systemy ogólne, trwały stan, zapis, integracja i wspólny runner | cały root z wyłączeniem `base_workbench/**`, `underwater_map_workbench/**` i `diver_workbench/**` | trzy warsztaty jako zależności tylko do odczytu | każde źródło należące do Bazy, Mapy albo Nurka |
| Baza: scena Przystani, budynki, lokalne zarządzanie, gospodarka, UI, definicje, dane, grafika i testy | wyłącznie `base_workbench/**` | publiczne kontrakty root, Mapy i Nurka tylko do odczytu | root, `underwater_map_workbench/**` oraz `diver_workbench/**` |
| Mapa: rejestr, globalne złożenie, wspólny builder/kompilator i grafika świata | `underwater_map_workbench/**` z wyłączeniem prywatnych źródeł istniejących `structures/<id>/`; builder może odtworzyć ich `generated/**` | pakiety struktur oraz publiczne kontrakty root, Bazy i Nurka tylko do odczytu | prywatne źródło jednego pakietu, root, `base_workbench/**` oraz `diver_workbench/**` |
| Struktura: prywatna topologia, grafika, runtime i testy jednego `structures/<id>/` | wyłącznie własne `underwater_map_workbench/structures/<id>/**`; kanoniczne uszczelnienie może zaktualizować tylko lokalny manifest | Mapa, inne pakiety, Root, Baza i Nurek tylko do odczytu | mapowy pin/rejestr, origin, wspólna Mapa, inny pakiet, root, Baza albo Nurek |
| Nurek: scena i prezentacja avatara | wyłącznie `diver_workbench/**` | publiczne kontrakty root, Bazy i Mapy tylko do odczytu | root, `base_workbench/**` oraz `underwater_map_workbench/**` |
...
- Test wnętrza jednej domeny należy do jej warsztatu. Test składający Root z Bazą, Mapą albo Nurkiem należy do root i sprawdza publiczne zachowanie, bez zamrażania prywatnej hierarchii węzłów, lokalnych fixture'ów Bazy, liczby klatek avatara, współrzędnych manifestu ani innych prywatnych szczegółów.
...
- Root lub koordynator przydziela jedno proste, niezależne zadanie jednemu agentowi. Zadanie implementacyjne zawsze uruchamia bezpośrednio jako **Worktree**, nigdy jako Local. `tools/start_agent_task.ps1` tworzy z aktualnego `origin/main` osobny pełny Git worktree i unikalną gałąź `codex/<owner>/<task-slug>-<id>`; ownerami są `root`, `base`, `map`, `diver`, `integration` oraz `structure-<id>`. Lokalny i GitHubowy fast-check maszynowo odrzucają diff wykraczający poza ownera gałęzi; sam osobny CWD albo gałąź `codex/*` we wspólnym głównym checkoutcie nie daje izolacji.
- Zadania zapisujące wspólny zestaw publikacji Mapy — `underwater_map_workbench/map_manifest.json`, `underwater_map_workbench/UnderwaterMap.tscn`, `underwater_map_workbench/assets/generated/**`, mapowe metadane builda, wspólny builder lub kompilator albo `underwater_map_workbench/structures/*/generated/**` — koordynator uruchamia i publikuje sekwencyjnie z aktualnego `origin/main`; kolejne takie zadanie zaczyna się dopiero po stanie `MERGED` albo `CLOSED` poprzedniego PR, a inne zadania o rozłącznych zapisach nadal pracują równolegle.
...
- Agent skupia się na zadaniu: `implementacja -> lokalne testy zadania -> lokalny fast-check`. Porażkę poprawia i powtarza właściwy etap. Po `PASS` kończy sekwencją `commit -> push + PR -> KONIEC`; operacyjnie jedno `tools/finish_agent_task.ps1` wykonuje logiczny commit i deleguje exact push + PR do `publish_agent_pr.ps1`, który dla zwykłej zmiany włącza squash auto-merge. Agent nie pushuje do `main`, nie polluje i nie babysituje kolejki. Nie mówi „gotowe” ani „wdrożone”, dopóki publisher nie potwierdzi `LocalHead = RemoteHead = PullRequestHead` dla dokładnie jednego otwartego PR.
...
- Przed pushem publisher pobiera świeży `origin/main` i wykonuje bezstanowy preflight merge. Konflikt zatrzymuje publikację przed pushem i wymaga nowego zadania z aktualnego `main`; bezkonfliktowy opóźniony branch może przejść do kolejki.
...
- Start następnego zadania może automatycznie posprzątać tylko stare worktree z exact scalonym PR-em. `cleanup_merged_worktrees.ps1` nigdy nie usuwa dirty, detached, otwartego, niescalonego ani przesuniętego worktree; takie katalogi jedynie raportuje.
...
`base_workbench/`, `underwater_map_workbench/` i `diver_workbench/` są trzema zatwierdzonymi wyjątkami domenowymi. Baza po atomowej migracji posiada aktywną scenę Przystani, lokalne UI, systemy budynków i zarządzania, definicje, dane, assety oraz testy Bazy. Mapa posiada aktywny manifest, generowaną scenę, builder, smoke test, shadery środowiska i worldowe assety podwodnego gameplayu; może zawierać podrzędne pakiety `structures/<id>/` zgodne z ARD-0106, ale nadal jest ich jednym właścicielem domenowym. Nurek posiada pojedynczą scenę avatara, jego adapter, fizyczną bryłę, grafikę, animację, sockety, shadery, VFX oraz lokalne testy i capture. Każdy warsztat ma własne `.ai/PROJECT_CONTEXT.md`, `.ai/DECISIONS.md`, `README.md` i `AGENTS.md`; struktura ma wyłącznie operacyjne `AGENTS.md` i `README.md`, bez własnego `.ai`. Nie wolno kopiować do warsztatów globalnych reguł gry, trwałego stanu ani persistence; root zachowuje przekrojowy kontrakt zgodnie z ARD-0102, ARD-0105, ARD-0106 i ARD-0114.
