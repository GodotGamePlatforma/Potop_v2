...
- Jednocześnie zapisujący agenci Rootu, Mapy, Nurka i każdego `structures/<id>/` pracują w osobnych pełnych Git worktrees oraz na gałęziach `codex/<owner>/<task-slug>` z jawnego commita integracyjnego potwierdzonego candidate receiptem, zgodnym pełnym run receiptem `PASS` i lokalnym refem `refs/last-green/integration`. Ref jest authority tylko dla linked worktrees jednego Git common-dir; nie jest refem zdalnym i nie przechodzi przez clone, fetch ani push. Sam osobny CWD nie izoluje wspólnego checkoutu. Bieżącego dirty checkoutu nie wolno automatycznie uznać za baseline ani kopiować do worktrees; nie blokuje on jednak materializacji exact HEAD/tree już wystawionego immutable receiptu.
- Pliki śledzone objęte atrybutem Git `eol=lf` muszą mieć w worktree wyłącznie LF. `python -B tools/workbench_contract.py --repo . eol-check`, wspólny runner i mapowy builder odrzucają `w/crlf` oraz `w/mixed`, ale dopuszczają zwykłą dirty zmianę zapisaną LF. Candidate create/verify wykonuje dodatkowo mocniejszą kontrolę surowych bajtów całego śledzonego drzewa względem exact HEAD/index; czysty status po clean filterze nie jest wystarczającym dowodem reprodukowalności.
...
- `setup_agent_worktree.ps1` kończy tworzenie worktree niezmiennym bundlem w `<git-common-dir>/codex-agent-assignments/v1/<task-id-hash>/<assignment-id>/`. Bundle przypina task, thread, owner, brief, destination, common-dir, branch, początkowy HEAD/tree, zamknięty i posortowany write-set, digesty candidate/run receiptów oraz termin ACK. Samo utworzenie oznacza wyłącznie `WAITING_ACK`; koordynator wysyła te same dane do wskazanego tasku, a agent przed jakimkolwiek zapisem wykonuje `assignment ack` z docelowego czystego worktree. Dopiero zweryfikowany ACK zmienia stan na `RUNNING`.
- ACK sprawdza exact top-level, common-dir, branch, początkowy HEAD/tree, clean status, thread, owner i digest write-setu. Błędny CWD, branch, rewizja, task/thread, owner, dirty worktree albo write-set są blokadą. Po rozpoczęciu pracy `validate --assignment <assignment-id> --task-id <task-id> --thread-id <thread-id> --diff` egzekwuje zarazem ownera i węższą allowlistę przydziału; samo `validate --owner` nie zastępuje aktywnego assignmentu.
- Timeout ACK nie zezwala na uruchomienie drugiego autora. Koordynator wykonuje `assignment redispatch` wyłącznie dla tego samego tasku, threadu, worktree i bundle'a z nowym terminem, a po hand-offie albo anulowaniu zapisuje niezmienne `assignment close`. `assignment gc` tworzy tylko plan retencji zamkniętych bundle'i i nigdy ich nie usuwa. Wszystkie zdarzenia korzystają ze wspólnego locka common-dir, są publikowane przez staging+rename i mają własny digest; tamper lub dwa aktywne, nakładające się write-sety są błędem fail-closed.
...
- Domyślny runner najpierw przechodzi fail-closed bramkę LF, następnie kopiuje pliki śledzone oraz niesledzone, których nie wykluczają standardowe reguły Git, i potwierdza `źródło przed == źródło po == kopia`. Każdy isolated run ma własne `.godot`, `user://`, logi, porty procesu i capture; równoległy Godot jest dozwolony tylko w osobnych pełnych workspace z jawnym `--path`. Runner odrzuca `-InPlace`, ponieważ nie tworzy ono kompletnej granicy snapshotu i danych użytkownika. Candidate/publication receipt potwierdza pliki, a osobny run receipt wiąże HEAD/tree, snapshot, overlay, runner, Godot, cele i wyniki; tylko ich zgodność oraz pełny `PASS` może przesunąć lokalny `refs/last-green/integration` przez Git CAS.
- Szybka walidacja gałęzi i pełna certyfikacja integracyjna są rozdzielone. `Agent branch validation` daje autorowi read-only feedback bez uruchamiania całej gry; pełny executor działa asynchronicznie na immutable SHA w czterech osobnych Windows jobach headless i jednym natywnym. Każdy shard ma własną VM/workspace. Równolegle osobny zaufany job wykonuje niedestrukcyjny check authority Mapy. Agregator przyjmuje dokładnie pięć receiptów shardów oraz jeden mapowy receipt z logiem, wszystkie związane przez exact candidate, plan, artifact ID, digest i run ID; kanoniczny receipt-set hashuje w ustalonej kolejności SHA-256 candidate receipt, agregatu Godota i mapowego receiptu, a jego digest trafia do jedynego `integration-green`. Limit 30 sekund jest celem szybkiej bramki, nie twardym limitem pełnej regresji Godota.
- Wewnątrz każdego shardu jeden importowany seed jest tylko źródłem kolejnych świeżych kopii per target. Proces Godota otrzymuje prywatne `.godot`, `user://`, TEMP/TMP i porty oraz środowisko bez tokenów, OIDC i plików komend GitHub; zaufany rodzic przechwytuje pipe stdout/stderr, domyślnie zapisuje FAIL, zamyka drzewo w Windows Job Object i wymaga pojedynczego completion record. Fingerprint wejść przed i po celu musi być identyczny, z wyjątkiem sidecarów `.uid` i `.import` utworzonych podczas izolowanego importu.
- GitHub-hosted job rozdziela współpracujących agentów i chroni przed kolizjami, ale Godot uruchomiony jako konto runnera nie jest sandboxem dla złośliwego kodu. Dlatego kandydat nie dostaje sekretów ani zapisu repozytorium, chroniony control-plane pochodzi z default branch, a silniejszy model zagrożeń wymaga osobnego restricted SID/DACL albo zewnętrznej guest VM.
- Po ukończonym bootstrapie App/environment/rulesetu i zielonym exact-main runie zwykły, niedraftowy PR z tego repozytorium na aktualnym `main` jest przyjmowany automatycznie po `Agent branch validation`. Zaufany kontroler z default branch działa wyłącznie przez API: ponownie wiąże exact base/head, owner branch, wynik szybkiej bramki oraz pełną listę zmienionych ścieżek, dodaje `integration-ready` i wysyła istniejący immutable handoff. Nie checkoutuje, nie pobiera artefaktów ani nie wykonuje kodu kandydata. Draft, fork, stara baza i chroniony control-plane nie są przyjmowane; po przesunięciu `main` autor wykonuje zwykły merge aktualnego `main`, testuje i pushuje nowy SHA. Pełna bramka, App-owned `integration-green`, exact-SHA merge i exact-main audit pozostają jedyną drogą publikacji.
- Zwykły lane zawsze odrzuca zmianę chronionego control-plane'u. Rzadki PR Mapy może zmienić od jednego do trzech plików wyłącznie z jawnej listy builder/portal-contract/smoke przez `map-control-plane-maintenance.yml`: właściciel ręcznie uruchamia workflow z exact PR/base/head i digestem przeglądu, zatwierdza jednoosobowy environment, kandydat przechodzi secretless pełną bramkę, a merge pozostaje ręczny. Handler zamkniętego PR automatycznie uruchamia exact-main audit. Nie łącz tej rewizji z runtime, manifestem, sceną, generated, dokumentacją ani zwykłym source PR; po zielonym main twórz osobny PR Mapy. Administracyjny bypass environment ma być wyłączony w konfiguracji GitHub.
- `main` ani lokalny ref ostatniego zielonego stanu nie przyjmuje czerwonego kandydata. Osobny worktree integracyjny składa jawny zestaw rewizji na najnowszej bazie i publikuje go dopiero po build/check, imporcie oraz wymaganych testach; integrator przekazuje oczekiwaną starą wartość refa, a `git update-ref` przesuwa go wyłącznie fast-forward po ponownej weryfikacji pełnego receiptu. Setup autora przed i po materializacji wymaga `candidate HEAD == run HEAD == refs/last-green/integration`; wyścig powoduje rollback nowego worktree. Weryfikacja receiptów i zmiana refa nie uruchamiają Godota, więc porażka nie blokuje producentów ani nie odbiera im zielonej podstawy. Lokalny pre-push guard blokuje agentom bezpośredni push do `main`; jego własna polityka musi działać przed uploadem i przekazać ten sam strumień rekordów do `git lfs pre-push`, nigdy zastępować standardowej bramki LFS. Obie części działają fail-closed. Zdalna branch protection/required checks jest obowiązkowa tam, gdzie pozwala na nią hosting i plan, a bez niej jeden integrator egzekwuje kolejkę proceduralnie.
