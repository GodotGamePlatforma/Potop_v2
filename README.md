...
Publiczne repo ma aktywny GitHub ruleset wymagający PR do `main`, rozwiązania wątków review oraz blokujący deletion i non-fast-forward bez bypassu. Bootstrap GitHub App/environment i strict pojedynczego App-owned `integration-green` jest zakończony, a `AUTO_INTEGRATOR_ENABLED=true`. Dla zwykłego PR z gałęzi `codex/*` nie dodawaj ręcznie `integration-ready`: pozostaw PR jako draft podczas authoringu, zaktualizuj go zwykłym merge'em do bieżącego `main`, zakończ testy i oznacz jako ready. Po zielonym `Agent branch validation` zaufany, API-only kontroler sam sprawdza exact base/head, tę samą gałąź i repo, brak chronionych ścieżek oraz najnowszy fast PASS, po czym dodaje etykietę i uruchamia istniejącą pełną bramkę. Pełny PASS publikuje `integration-green`, scala exact SHA metodą merge i uruchamia exact-main audit. Draft, fork, nieaktualna baza albo control-plane są jawnie pomijane; lokalne assignment/candidate/run digesty nadal nie zastępują zdalnego status checku.
