...
func medical_care_projection(state) -> Dictionary:
	...
	var definition = ResourceLoader.load("res://data/buildings/infirmary.tres")
...
func ration_forecast(state) -> Dictionary:
	...
	var kitchen_definition = ResourceLoader.load("res://data/buildings/kitchen.tres")
	var capable: Array = []
...
func _workshop_recipe_names(level: int, exact_level: bool) -> Array[String]:
	var result: Array[String] = []
	for file_name in DirAccess.get_files_at("res://data/workshop_recipes"):
		if not (file_name.ends_with(".tres") or file_name.ends_with(".res")):
			continue
		var recipe = ResourceLoader.load("res://data/workshop_recipes".path_join(file_name))
		if recipe == null:
			continue
