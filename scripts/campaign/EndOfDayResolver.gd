...
func _building_workforce(state, building, bonus_id: String) -> Dictionary:
	...
	if building != null:
		definition = ResourceLoader.load("res://base_workbench/data/buildings/%s.tres" % building.definition_id)
		var building_id := str(building.id)
...
func _building_display_name(building) -> String:
	if building == null:
		return "Budynek"
	var definition = ResourceLoader.load("res://base_workbench/data/buildings/%s.tres" % str(building.definition_id))
