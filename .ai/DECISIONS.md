...
- D11. `main` i ewentualny ref `last-green` moze przesunac tylko integrator po zlozeniu kandydata na najnowszej bazie i przejsciu wymaganych checkow. Lokalny pre-push guard blokuje bezposredni push agentow do `main` oraz non-fast-forward/force-push rewizji `codex/*`; obejscie proceduralne integratora wymaga jawnego trybu i zgodnego run receiptu.
- D12. Ochrona zdalna jest warstwa dodatkowa, nie zalozeniem runtime narzedzi lokalnych. Gdy hosting i plan pozwalaja, `main` wymaga PR, strict status checks, CODEOWNERS i braku bypassu; merge queue wymaga ponownego testu na najnowszym target branch. Gdy hosting tego nie egzekwuje, jeden integrator kolejkuje rewizje sekwencyjnie, a lokalny hook jest jawnie slabsza gwarancja proceduralna.
