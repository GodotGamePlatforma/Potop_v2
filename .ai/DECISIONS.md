...
- D2. Zaufany workflow z wersji domyslnej repozytorium przygotowuje plan, narzedzia walidacji i liste chronionych sciezek. Kod kandydata nie moze zastapic control-plane'u, planu shardow, agregatora ani weryfikatora podczas wlasnego przebiegu. Zmiana chronionego workflow lub `tools/ci_*` przechodzi jawna polityke i ten sam zaufany preflight.
...
- D9. Agregator akceptuje wylacznie dokladny zestaw pieciu receiptow, zgodne HEAD/tree/snapshot/overlay/runner/Godot/plan i terminalny `PASS` kazdego celu. Jedynym publicznym required-check contextem jest `integration-green`, publikowanym dokladnie raz przez odseparowany GitHub App attester po weryfikacji agregatu; zwykly job ani kandydat nie publikuje drugiego checku o tej nazwie.
- D10. Uprawnienia sa rozdzielone: shardy sa read-only i bez sekretow, publisher ma tylko metadane oraz `checks:write`, a osobny merge krok uzywa zwyklego minimalnego `GITHUB_TOKEN`. GitHub App nie otrzymuje `Administration:write`; tozsamosc app, ruleset bez bypassu i strict wymaganie `integration-green` sa konfigurowane i audytowane przez administratora poza przebiegiem kandydata.
...
- Powod i skutek: ciezkie testy wykorzystuja piec rozlacznych maszyn i przekazuja male, weryfikowalne wyniki, dzieki czemu agenci zachowuja szybka petle pracy bez wspoldzielenia Godota lub checkoutu. Cena jest odroczony pelny wynik, koszt pieciu jobow, jawny bootstrap GitHub App/rulesetu oraz brak deklaracji sandboxu wobec zlosliwego kodu.
- Odwolania: ARD-0108; ARD-0109; AGENTS.md; README.md; docs/Ostatni_Pomost_architektura_Godot.txt - sekcje 2 i 13; `.github/workflows/agent-validation.yml`; `.github/workflows/agent-integration.yml`; `.github/workflows/agent-auto-integrator.yml`.
