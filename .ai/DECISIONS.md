...
- Status / aktywny zakres: Czesciowo zastapione; D1-D3,D5-D8,D10,D12-D14 pozostaja aktywne
- Zatwierdzenie: 2026-08-26
- Relacje: Doprecyzowuje ARD-0099/D4, ARD-0102/D1-D2,D8-D10, ARD-0105/D1,D5-D7 i ARD-0106/D1-D3,D6-D9 | Zastapiona przez: ARD-0112/D1-D10 w D4,D9,D11
...
- Status / aktywny zakres: Czesciowo zastapione; D2-D6,D8,D10 pozostaja aktywne
- Zatwierdzenie: 2026-08-27
- Relacje: Doprecyzowuje ARD-0108/D1-D4,D9-D12 | Zastapiona przez: ARD-0112/D1-D10 w D1,D7,D9
...
- Zatwierdzenie: 2026-08-27
- Relacje: Doprecyzowuje ARD-0108/D2,D4,D6-D12 i ARD-0109/D9-D10 | Zastapiona przez: ARD-0112/calosc
...
## ARD-0112 - PR przechodzi szybka zaufana bramke i kolejke, a grywalny build jest promowany osobno

- Domena: proces agentow, Git worktrees, GitHub Actions, merge queue, automatyczne scalanie i lokalny build
- Status / aktywny zakres: Obowiazuje; D1-D10
- Zatwierdzenie: 2026-08-27T21:42:01Z; wariant CI-S1A zatwierdzony bezpośrednią wiadomością użytkownika `01a0452c-3923-7292-8c98-447213a5cecc` po przeniesieniu publicznego repozytorium do organizacji `GodotGamePlatforma`
- Relacje: Zastepuje ARD-0108/D4,D9,D11, ARD-0109/D1,D7,D9 i ARD-0110/calosc | Zastapiona przez: brak
- D1. Kazdy agent zaczyna od aktualnego `origin/main` w osobnym linked Git worktree i na galezi `codex/<owner>/<task-slug>`. `setup_agent_worktree.ps1 -FromOriginMain` nie wymaga candidate/run receiptow ani lokalnego LKG, ale wymaga `TaskId`, `ThreadId`, briefu i zamknietego `WriteSet`; jako ostatni marker tworzy jeden trwaly assignment schema v2 w stanie `WAITING_ACK`. Dopiero zgodny ACK ustanawia `RUNNING`. To upraszcza wybor bazy, nie usuwa identity, write-setu ani aktywnych ARD-0109/D2-D6,D8,D10.
- D2. `publish_agent_pr.ps1` wymaga czystego worktree, aktualnej bazy `origin/main`, exact HEAD oraz dokladnie jednego zgodnego assignmentu `RUNNING`; wykonuje assignment-scoped owner/write-set validation, ponownie sprawdza niezmiennosc HEAD i bazy, pushuje tylko biezaca galaz oraz tworzy zwykly PR przypiety do tego HEAD. Kazdy PR otrzymuje `--auto --squash --match-head-commit`; nie istnieje reczny wariant control-plane'u, bezposredni push do `main`, custom merge ani bypass.
- D3. Kandydacki workflow dziala bez sekretow dla `pull_request` i `merge_group` oraz daje szybki feedback. Publisher wysyla exact PR/base/head przez `repository_dispatch verify-fast-pr`; receiver jest ladowany wylacznie z default branch i ponownie wiaze repo, biezacy PR albo merge-group, pelny paginowany diff, ownera, EOL i lekkie kontrakty. Wymagany `fast-green` publikuje dopiero zaufana GitHub App, ktorej klucza kandydat nie otrzymuje. Celem operacyjnym pozostaje 30-60 sekund.
- D4. Zaufany klasyfikator z default branch dzieli zmiany na zwykle i wrazliwe; caly control-plane jest zawsze wrazliwy. Zwykla zmiana uzyskuje App-owned `fast-green` po deterministycznej bramce. Wrazliwa zmiana przechodzi dodatkowo automatyczny Codex review na exact PR head, uruchomiony przez zaufana definicje bez wykonywania kodu kandydata w jobie z sekretem; diff jest wylacznie niezaufanymi danymi, proces nie ma sudo i zwraca strukturalny `PASS` albo `FAIL`. `OPENAI_API_KEY` nalezy wylacznie do zaufanego joba review i nie jest udostepniany workflow ani procesowi kandydata. Nie ma recznego review, merge, environment approval ani bypassu.
- D5. Ruleset `main` wymaga PR, App-owned `fast-green` oraz natywnej `merge_queue` z `merge_method=SQUASH`, `grouping_strategy=ALLGREEN`, pojedynczym PR-em w grupie i ograniczona liczba rownoleglych buildow. Po sukcesie PR-head receiver obserwuje pierwszy wpis kolejki, rozwiazuje exact syntetyczny merge-group SHA i wysyla kolejny zaufany dispatch. Kazde pozniejsze odtworzenie grupy po ruchu `main` wykrywa niezalezny, ladowany z default branch watchdog `schedule` (z `workflow_dispatch` tylko jako recovery): czyta wylacznie live queue refs i API PR, nie zalezy od workflow kandydata, nie pobiera artefaktow ani nie wykonuje jego kodu, po czym idempotentnie redispatchuje kazdy aktualny SHA. Brak App-owned checku na dowolnym aktualnym merge-group SHA fail-closed zatrzymuje kolejke. Success scala squash do `main`, failure usuwa wpis z kolejki i wymaga nowego commita. Control-plane przechodzi dokladnie ten sam automatyczny tor co pozostale wrazliwe zmiany; PR laczacy trust-root z runtime jest automatycznie odrzucany i musi zostac rozdzielony na dwa PR-y.
- D6. Po kazdym squash merge i pushu do `main` `.github/workflows/agent-integration.yml` uruchamia asynchronicznie niedestrukcyjny check Mapy oraz cztery shardy headless i jeden natywny. GitHubowy raport moze koaleskowac do najnowszego `main`; jest informacyjny, nie required dla kolejnego PR, nie wykonuje auto-revertu i nie zatrzymuje agentow. Czerwony `main` jest dozwolony i nie zmienia lokalnego ostatniego grywalnego wydania.
- D7. Dedykowany lokalny source clone jest czystym, standalone mirrorem `main`: pobiera i ustawia exact SHA z `origin/main`, ale nigdy nie jest katalogiem gry, wspolnym worktree ani miejscem authoringu. Jeden lokalny runner utrzymuje trwala kolejke wszystkich nowych main SHA, przetwarza je sekwencyjnie bez anulowania i po restarcie wznawia pierwszy nieterminalny element bez pominiecia rewizji.
- D8. Kazdy main SHA jest sprawdzany we wlasnym immutable katalogu. Runner przypina exact commit, wykonuje `git lfs fetch`/checkout/fsck, mapowy check, realny eksport/build, pelny izolowany zestaw testow i smoke wyeksportowanego programu potwierdzajacy utworzenie oraz zaladowanie mapy. Dopiero caly PASS atomowo przelacza wskaznik `current` na immutable build `<sha>`; FAIL zapisuje czerwony `full-green` i alert, ale absolutnie nie zmienia `current`. Brak `export_presets.cfg`, wymaganych JSON w eksporcie albo produkcyjnego smoke powoduje `EXPORT_BOOTSTRAP_REQUIRED`, a nie pozorny grywalny build.
- D9. Receipty, LKG i inne dowody moga powstawac wewnetrznie w CI i lokalnym builderze jako diagnostyka exact SHA. Nie sa recznym wejsciem codziennego authoringu ani publikacji PR. Assignment i ACK pozostaja automatyczna, trwala koordynacja worktree/write-setu zgodnie z D1, nie certyfikacja produktu.
- D10. Repozytoryjny commit tylko przygotowuje control-plane. Aktywacja wymaga osobnego, atomowego bootstrapu po immutable review: instalacji App `potop-v2-integration-attester` w organizacji i ograniczenia jej do tego repo, potwierdzenia `checks:write` oraz App ID 4737404, automatycznego environment `integration-attester` bez reviewerow i bypassu oraz bezpiecznego probe obecnosci `INTEGRATION_ATTESTER_PRIVATE_KEY` i `OPENAI_API_KEY`. `AUTO_INTEGRATOR_ENABLED` pozostaje false do potwierdzenia tych zaleznosci; potem moze przejsc true tylko przy nadal wymaganym starym `integration-green`, aby opublikowac pierwszy niewymagany App-owned `fast-green` i PR-head canary bez otwarcia `main`. Kolejny fail-closed krok dodaje `fast-green` jako drugi strict required check, wlacza queue/squash i wykonuje merge-group canary oraz exact-main regresje; dopiero ich sukces usuwa stary check. Exact JSON i SHA-256 rulesetu sa zachowane przed i po; awaria przywraca poprzedni dokument i AUTO=false. Ten task nie zmienia rulesetu, sekretow, runnera, merge ani `main`.
- Powod i skutek: agenci pracuja niezaleznie i czekaja tylko na szybka bramke oraz natywna kolejke, a ciezkie testy i eksport nie blokuja kolejnych PR-ow. Najnowszy kod na `main` jest swiadomie oddzielony od ostatniego zweryfikowanego, grywalnego `current`. Cena to wymagany zaufany App/Codex control-plane, koszt przetworzenia kazdego merge SHA i jawny bootstrap eksportu przed pierwsza promocja grywalnego builda.
- Odwolania: ARD-0108; ARD-0109; AGENTS.md; README.md; docs/Ostatni_Pomost_architektura_Godot.txt - sekcje 2 i 13; `.github/workflows/agent-validation.yml`; `.github/workflows/agent-auto-integrator.yml`; `.github/workflows/agent-integration.yml`; `tools/setup_agent_worktree.ps1`; `tools/publish_agent_pr.ps1`; `tools/sync_play_main.ps1`.
