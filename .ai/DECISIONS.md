...
- D14. Certyfikowany maintenance PR jest scalany recznie. Zaufany handler `pull_request: closed` ponownie wymaga od jednego do trzech unikalnych plikow z tej samej zamknietej allowlisty, poprawnej galezi i labela, a najnowszy App-owned check tej exact rodziny `PR/base/head` musi byc zakonczonym sukcesem `control-plane-v2`; starszy sukces nie przykrywa nowszego queued, running ani failure. Handler potwierdza ancestry aktualnego `main` i dopiero wtedy wysyla idempotentny `verify-integrated-main`. Zielony exact-main audyt ustanawia nowe zdalne LKG przed zwyklym PR Mapy. Check pozostawiony jako `queued` jest bezpiecznie redispatchowany, `in_progress` oznacza przyjety lease, a awaria dispatchu konczy ten sam check porazka zamiast pozostawienia wiszacej zgody.
