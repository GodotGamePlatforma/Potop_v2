...
- D1. Root albo koordynator przydziela jedno proste, niezalezne zadanie jednemu agentowi. Zadanie ma jeden pelny Git worktree, jedna galaz `codex/<owner>/<task-slug>` i jeden logiczny PR. Autor wykonuje `implementacja -> lokalne testy zadania -> lokalny fast-check`; po porazce poprawia zmiane, a po `PASS` wykonuje `commit -> tools/publish_agent_pr.ps1 (push + PR) -> KONIEC`. Brak commitu albo opublikowanego PR oznacza zadanie niedostarczone. Autor nie pushuje bezposrednio do `main`, nie polluje i nie babysituje kolejki.
...
## ARD-0114 - Warsztat Bazy jest jedynym authority prezentacji i lokalnej domeny Przystani

- Domena: Baza, zarzadzanie Przystania, prezentacja, dane definicyjne, testy i routing pracy
- Status / aktywny zakres: Obowiazuje; D1-D9
- Zatwierdzenie: 2026-08-29
- Relacje: Doprecyzowuje ARD-0002/D1-D2, ARD-0018, ARD-0066/D2 i ARD-0113 | Zastapiona przez: brak
- D1. `base_workbench/` jest trzecim zatwierdzonym warsztatem domenowym wewnatrz tego samego projektu Godot. Nie posiada osobnego `project.godot`, InputMap, ustawien fizyki, cache projektu, formatu zapisu ani kopii ogolnych systemow; uruchomienie i testy zawsze korzystaja z projektu nadrzednego.
- D2. Warsztat Bazy jest jedynym wlascicielem aktywnej `runtime/BaseScene.tscn`, jej lokalnych kontrolerow i srodowiska, scen oraz skryptow UI pod `ui/`, systemow budynkow i zarzadzania pod `systems/`, definicji i danych Bazy, jej grafiki, audio, narzedzi zrodlowych oraz lokalnych testow i capture'ow. Pliki sa ladowane bezposrednio spod `res://base_workbench/`; root nie utrzymuje kopii pod `scenes/base`, `scripts/base`, `assets/base*`, `assets/audio/base`, `data/buildings`, `data/events` ani `data/workshop_recipes`.
- D3. Root pozostaje wlascicielem `GameState`, trwalych stanow domenowych, `SaveManager`, walidacji i migracji, atomowego rozliczenia dnia, kampanii, misji, difficulty, pogody, rozwoju i chorob ocalencow, ogolnych systemow nurkowania oraz integracji scen. Baza konsumuje te publiczne kontrakty i nie tworzy ich drugiego modelu, formatu ani kopii stanu.
- D4. `base_workbench/{AGENTS.md,README.md,.ai/PROJECT_CONTEXT.md,.ai/DECISIONS.md}` jest jedynym zatwierdzonym lokalnym zestawem dokumentow Bazy. Dokumenty lokalne opisuja routing, ownership, biezacy stan warsztatu i prywatne decyzje implementacyjne, ale nie przejmuja globalnych regul produktu, kampanii, persistence ani przekrojowej architektury.
- D5. Prywatne zadanie sceny, budynku, systemu, definicji, danych, UI, assetu albo testu Bazy zapisuje wylacznie `base_workbench/**` i moze czytac publiczne kontrakty root, Mapy i Nurka. Zmiana zachowania widocznego dla gracza, trwalego stanu, kolejnosci rozliczenia, publicznej granicy Root-Baza albo wiecej niz jednego wlasciciela jest integracja prowadzona z root.
- D6. Test wewnetrzny Bazy znajduje sie bezposrednio w `base_workbench/tests/` i chroni lokalne systemy, scene, UI, assety i prezentacje. Root zachowuje testy persistence, kampanii, rozliczenia dnia oraz publicznego zlozenia `GameRoot -> BaseScene`, bez zamrazania prywatnej hierarchii UI i lokalnych fixture'ow. Wspolny runner przyjmuje bezposrednie cele `.gd` i `.tscn` warsztatu oraz nadal uruchamia Godota sekwencyjnie w izolowanej pelnej kopii projektu.
- D7. Pierwsze przeniesienie jest refaktorem bez zmiany gameplayu, balansu, widocznego zachowania, atomowosci dnia, `GameState`, rewizji kampanii, persistence, migracji i podpisu mapy. Zachowuje UID zasobow i znaczenie wszystkich stabilnych ID; nie aktywuje nieuzywanego `BuildingBlueprintSystem` ani historycznych hookow.
- D8. Warsztat jest operacyjnie kompletnym miejscem pracy nad Baza, lecz nie niezalezna biblioteka. Agent pracuje z CWD `base_workbench/` w pelnym checkoutcie i korzysta z nadrzednego `project.godot` oraz publicznych zaleznosci root. Integracja odbywa sie przez zwykly Git i jeden PR, a nie przez kopiowanie plikow miedzy workspace'ami.
- D9. Granica jest chroniona przez brak dawnych authority Bazy, dokladnie cztery lokalne dokumenty, pojedynczy projekt Godot, lokalne testy oraz rootowy test publicznego zlozenia. Zwykly autor wykonuje celowane testy Bazy i osobny lokalny fast-check, a po `PASS` publikuje exact commit zgodnie z ARD-0113; pelna regresja nalezy do merge queue.
- Powod i skutek: agent Bazy otrzymuje jeden uporzadkowany katalog z runtime, UI, systemami, definicjami, danymi, assetami, testami i lokalnym kontekstem, bez przejmowania globalnego stanu oraz zapisu. Cena jest jawna granica integracji z root i koniecznosc osobnego zakresu przy kazdej zmianie przekrojowej.
- Odwolania: AGENTS.md; base_workbench/AGENTS.md; base_workbench/.ai/DECISIONS.md; docs/Ostatni_Pomost_architektura_Godot.txt - sekcje 2, 4, 5, 11-13; README.md.
