...
- D1. Root albo koordynator przydziela jedno proste, niezalezne zadanie jednemu agentowi. Zadanie ma jeden pelny Git worktree, jedna galaz `codex/<owner>/<task-slug>` i jeden logiczny PR. Autor wykonuje `implementacja -> lokalny fast-check PASS -> commit -> push/PR + GitHub merge when ready (squash) -> KONIEC`; nie pushuje bezposrednio do `main`, nie polluje i nie babysituje kolejki.
- D2. Lokalny fast-check w worktree przed commitem i wymagany GitHub `fast-check` po utworzeniu PR sa dwoma roznymi przebiegami. Tylko jawny `PASS` GitHub fast-checku dla dokladnego headu PR pozwala dodac go do merge queue; kazdy inny wynik blokuje kolejke i merge, takze dla rzadkiego PR control-plane. Pelna bramka `integration-green` uruchamia sie wylacznie na merge group skladajacej aktualny `main` i dany PR.
...
- D7. GitHub chroni workflowy, narzedzia CI, konfiguracje `integration-green` i control-plane buildera. Zwykly PR nie zmienia tych sciezek, a rzadka zmiana korzysta z osobnego PR i recznej zgody wlasciciela; nie wymaga drugiego agenta oceniajacego pierwszego. Reczna zgoda jest dodatkowa bramka i nie omija wymaganego GitHub `fast-check PASS` ani pelnego `integration-green` merge group.
...
- D9. Zwykle przydzielenie jednego zadania przez root nie jest formalnym protokolem assignment. Bundle/store assignmentu, ACK, lokalne LKG, candidate/run receipty, FROZEN hand-offy, App attester, auto-integrator, agenci-audytorzy i rozmowy miedzy agentami nie naleza do procesu. Root moze koordynowac wynik lub konflikt, lecz autor starego PR nie jest wznawiany jako jego staly opiekun. Szczegoly wykonania CI pozostaja wymienne, o ile zachowuja D2-D8.
