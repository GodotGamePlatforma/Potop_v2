...
- Status / aktywny zakres: Czesciowo zastapione; D2-D4, D7-D9
- Zatwierdzenie: 2026-08-14
- Relacje: Zastepuje: ARD-0019/D2; ARD-0045/D3; ARD-0059/D2; ARD-0063/D4; ARD-0065/D2-D3; ARD-0068/D5; ARD-0073/D8; ARD-0076/D5; ARD-0082/D7; ARD-0083/D3,D5-D6; ARD-0085/D5; ARD-0088/calosc; ARD-0089/D5 | Zastapiona przez: ARD-0096/D1,D5; ARD-0111/D6
...
- D4. Jeden walidator wykonuje preflight oraz pelny postflight kandydata dokladnie biezacej rewizji. Zwykly zapis tej samej kampanii pracuje na odseparowanym kandydacie, ponownie odczytuje pending, zachowuje tylko poprawny primary jako backup i promuje pending; nie istnieje warstwa reinterpretacji legacy. `replace_campaign()` jest jedynym jawnym wyjatkiem i podlega ARD-0111/D3-D5.
- D5. Zastapiona przez ARD-0096/D5.
- D6. Zastapiona przez ARD-0111/D6.
...
## ARD-0108 - Rownolegli autorzy pracuja w osobnych worktrees, a integrator publikuje zweryfikowany wynik

- Domena: proces wytwarzania, granice warsztatow, Git, buildy, testy i publikacja pochodnych
- Status / aktywny zakres: Obowiazuje; D1-D14
- Zatwierdzenie: 2026-08-26
- Relacje: Doprecyzowuje ARD-0099/D4, ARD-0102/D1-D2,D8-D10, ARD-0105/D1,D5-D7 i ARD-0106/D1-D3,D6-D9 | Zastapiona przez: brak
- D1. Kazdy jednoczesnie zapisujacy autor Rootu, Mapy, Nurka albo jednego `structures/<id>/` pracuje w osobnym pelnym Git worktree i na galezi `codex/<owner>/<task-slug>` wyprowadzonej z jawnego, zweryfikowanego commita integracyjnego. Osobny CWD w tym samym worktree nie jest izolacja. Reczne uruchomienie gry lub edytora korzysta z osobnego worktree przeznaczonego do playtestu.
- D2. Worktree producenta moze byc czerwone i rozwijac nastepna rewizje bez blokowania innych. Producent tworzy male logiczne commity, pobiera aktualne referencje przez `fetch --prune` i pushuje wylacznie wlasna galaz `codex/*`, nigdy bezposrednio `main`. Do integracji trafia wylacznie niezmienny commit producenta albo zweryfikowana rewizja FROZEN z zamknieta lista plikow i receipt; integrator nigdy nie buduje z ruchomego katalogu autora. Po hand-offie exact SHA tej rewizji nie wolno rebase'owac ani force-pushowac; poprawka otrzymuje nowy commit, SHA i receipt.
- D3. Maszynowy kontrakt write-set rozpoznaje wlascicieli `root`, `map`, `diver`, `structure:<id>` i `integration` oraz odrzuca zapis poza zadeklarowana allowlista. Producent struktury uszczelnia tylko prywatny manifest i zrodla pakietu; Mapa odczytuje ten wynik i jako jedyna aktualizuje mapowy pin, globalne zlozenie oraz pochodne. Zaden krok producenta nie zapisuje obu authority.
- D4. Publication/candidate receipt dowodzi jedynie niezmiennego HEAD, tree, zamknietego zestawu plikow i ich digestow; nie jest dowodem zielonych testow. Osobny run receipt wiaze z tym samym HEAD/tree dokladny snapshot, overlay testowy, wersje runnera i Godota, zamkniety zestaw celow oraz ich wyniki. Commit wolno nazwac `last-green` dopiero po fail-closed weryfikacji obu receiptow i pelnej wymaganej bramki; targeted PASS nie wystarcza. Lokalnym authority tej tozsamosci jest dokladnie `refs/last-green/integration`, wspolny dla linked worktrees jednego Git common-dir.
- D5. Obliczenia builda powstaja poza blokada ze sealed inputu. Publikacja wspolnego zestawu manifestu, pinow, sceny i metadanych wykonuje ponowny hash wejsc oraz compare-and-swap pod jedna krotka blokada wyprowadzona ze wspolnego katalogu Git, a wiec wspolna dla linked worktrees. Atomowa podmiana pojedynczej sciezki nie jest traktowana jako atomowosc calego wieloplikowego zestawu.
- D6. Celowany test jednego warsztatu nie wykonuje globalnego discovery, refreshu ani promocji innej domeny. Pelne discovery pakietow, zlozenie Root-Mapa-Nurek i pelna regresja naleza wyłącznie do lane'u integracyjnego.
- D7. Kazda instancja Godota wskazuje jawny root projektu przez `--path` oraz ma prywatny cache projektu, `user://`, logi, capture i pozostale artefakty procesu. Rownolegle instancje sa dozwolone tylko w odseparowanych pelnych workspace; wspolny runner odrzuca `-InPlace`, poniewaz nie daje kompletnej granicy snapshotu i danych uzytkownika.
- D8. Snapshot testowy obejmuje wszystkie pliki sledzone oraz niesledzone, ktore nie sa ignorowane standardowymi regulami Git, i wymaga zgodnosci fingerprintu `zrodlo przed == zrodlo po == kopia`. Cache, ignorowane importy i artefakty uruchomienia nie sa wejsciem kompilacji ani przyczyna uniewaznienia snapshotu.
- D9. Obecny dirty checkout nie moze stac sie automatycznie baseline'em nowych worktrees ani wejsc do ich drzewa. Nie moze jednak blokowac materializacji exact HEAD/tree juz wystawionego immutable receiptu: bootstrap weryfikuje obiekty Git niezaleznie od stanu wywolujacego checkoutu. Narzedzie przygotowania worktrees domyslnie tylko pokazuje plan, a utworzenie wymaga jawnego candidate receiptu, zgodnego pelnego run receiptu `PASS` oraz rownosci ich HEAD z `refs/last-green/integration`. Ref jest odczytywany przed weryfikacja i ponownie po materializacji; zmiana lub rozjazd powoduje fail-closed rollback. Historyczna rewizja bez resolvera LKG nie jest automatycznie migrowana: pierwszy baseline po tej decyzji musi przejsc nowa pelna bramke i promocje.
- D10. Agent poznaje cudza prace przez remote refs, PR-y i receipty po `fetch`, nigdy przez live worktree innego autora. Zaleznosc miedzy zadaniami wskazuje exact commit SHA albo jawny stacked PR, nie ruchomy tip galezi. Push jest backupem i hand-offem, a nie dowodem integracji.
- D11. `main` i `refs/last-green/integration` moze przesunac tylko integrator po zlozeniu kandydata na najnowszej bazie i przejsciu wymaganych checkow. Resolver ponownie weryfikuje exact candidate HEAD/tree i pelny run receipt, wymaga fast-forward oraz zmienia ref atomowym `git update-ref <new> <expected-old>`; niezgodny expected-old, czerwony/targeted run albo non-fast-forward nie zmienia refa, a ruch refa podczas weryfikacji nie moze zostac uznany za sukces i wymaga ponowienia od nowo odczytanej wartosci. Weryfikacja i CAS nie uruchamiaja Godota ani nie zajmuja lane'u authoringu. Lokalny pre-push guard blokuje bezposredni push agentow do `main` oraz non-fast-forward/force-push rewizji `codex/*`; obejscie proceduralne integratora wymaga jawnego trybu i zgodnego run receiptu.
- D12. Ochrona zdalna jest warstwa dodatkowa, nie zalozeniem runtime narzedzi lokalnych. `refs/last-green/integration` jest refem lokalnym wspolnego Git common-dir i nie replikuje sie przez clone, fetch ani push; zdalny LKG wymagalby osobnego chronionego refspecu albo authority CI i nie jest tu wdrozony. Gdy hosting i plan pozwalaja, `main` wymaga PR, strict status checks, CODEOWNERS i braku bypassu; merge queue wymaga ponownego testu na najnowszym target branch. Gdy hosting tego nie egzekwuje, jeden integrator kolejkuje rewizje sekwencyjnie, a lokalny hook jest jawnie slabsza gwarancja proceduralna.
- Powod i skutek: prywatna iteracja przestaje byc globalna transakcja, a integrator czeka tylko na konkretna niezmienna rewizje zaleznosci, nie na koniec pracy jej autora. Cena jest obowiazek pracy na galeziach, jawnego hand-offu i jednego kontrolowanego punktu publikacji wspolnych pochodnych.
- Odwolania: ARD-0048; ARD-0099; ARD-0102; ARD-0105; ARD-0106; docs/Ostatni_Pomost_architektura_Godot.txt - sekcje 2, 9.1 i 13; AGENTS.md.

## ARD-0109 - Trwaly assignment i ACK lacza task z jednym worktree oraz zamknietym write-setem

- Domena: koordynacja agentow, Git worktrees, ownership, hand-off i audyt procesu
- Status / aktywny zakres: Obowiazuje; D1-D10
- Zatwierdzenie: 2026-08-27
- Relacje: Doprecyzowuje ARD-0108/D1-D4,D9-D12 | Zastapiona przez: brak
- D1. Samo utworzenie worktree ani wyslanie wiadomosci nie uruchamia autora. Koordynator tworzy dokladnie jeden niezmienny assignment dla jednego `task_id`, wskazanego `thread_id`, ownera, briefu, destination, Git common-dir, galezi, poczatkowego HEAD/tree, zamknietego write-setu, digestow candidate/run receiptow i terminu ACK. Stan po utworzeniu to `WAITING_ACK`; dopiero poprawny ACK tego samego tasku ustanawia `RUNNING`.
- D2. Bundle jest przechowywany poza checkoutami w `<git-common-dir>/codex-agent-assignments/v1/<task-id-hash>/<assignment-id>/`. Rekord assignmentu oraz osobne zdarzenia ACK, redispatch i close sa publikowane przez prywatny staging i rename pod jednym wspolnym lockiem common-dir, maja kanoniczny digest i nie sa nadpisywane. Status jest wyprowadzany z tych niezmiennych zdarzen; tamper, luka sekwencji albo niezgodny digest powoduje fail-closed.
- D3. ACK wykonany z docelowego CWD ponownie sprawdza exact top-level, common-dir, branch, poczatkowy HEAD/tree, clean status, `thread_id`, owner oraz identyczny, posortowany write-set i jego digest. Zly katalog, branch, rewizja, thread, owner, dirty stan albo rozszerzona lista zapisu nie moga przejsc do `RUNNING`.
- D4. Jeden `task_id` ma przez caly cykl najwyzej jeden bundle. Ponowienie tego samego, bajtowo identycznego create jest idempotentnym odczytem zwyciezcy; inna proba tego tasku jest odrzucana. Dwa niezamkniete assignmenty nie moga miec rownych ani hierarchicznie nakladajacych sie sciezek write-setu, niezaleznie od ownera i osobnych worktrees.
- D5. Timeout ACK nie zwalnia write-setu i nie upowaznia drugiego autora. `redispatch` moze jedynie przedluzyc termin tego samego assignmentu dla tego samego tasku, threadu, destination, branch i rewizji; tworzy kolejne niezmienne zdarzenie z lancuchem deadline'ow. Po hand-offie lub anulowaniu `close` zwalnia write-set przez osobne zdarzenie. Plan retencji obejmuje tylko dostatecznie stare, zamkniete bundle i sam niczego nie usuwa.
- D6. W trakcie authoringu maszynowa bramka `validate --assignment ... --diff` wymaga stanu `RUNNING`, zgodnego tasku/threadu/ownera, tego samego destination/common-dir/branch i potomka poczatkowego HEAD. Kazda zmieniona sciezka musi jednoczesnie nalezec do ownera oraz exact sciezki albo poddrzewa jawnie wymienionego w assignment write-set; owner `integration` nie rozszerza automatycznie listy assignmentu.
- D7. `setup_agent_worktree.ps1` materializuje wyłącznie zweryfikowany LKG, przechodzi doctor i dopiero jako ostatni marker tworzy assignment. Porażka przed tym markerem wycofuje wylacznie worktree i galaz utworzone przez dana probe. Po sukcesie bundle pozostaje `WAITING_ACK`, a koordynator musi wyslac do konkretnego tasku exact destination, branch, HEAD/tree, owner, brief i write-set; brak ACK oznacza ponowienie dispatchu tego samego tasku, nigdy uruchomienie drugiego autora.
- D8. Alias ownera `play` nie istnieje. Ręczny edytor lub playtest nadal korzysta z osobnego worktree, lecz jego zakres otrzymuje rzeczywistego ownera domenowego albo `integration` i podlega temu samemu assignmentowi oraz write-setowi.
- D9. Candidate i run receipty uzyte przy assignment musza wskazywac ten sam poczatkowy HEAD/tree i pelny `PASS`, a ich surowe SHA-256 sa czescia bundle'a. Assignment nie przesuwa LKG, nie wykonuje testow i nie certyfikuje przyszlych commitow autora; hand-off nadal wymaga nowego exact SHA, testow i receiptow zgodnie z ARD-0108.
- D10. Lokalny bundle nie jest GitHub attestation ani wynikiem zdalnego required checku. Ochrona `main`, PR i status `integration-green` sa osobna warstwa hostingu; ich wlaczenie lub awaria nie zmienia lokalnego stanu assignmentu i nie moze byc wnioskowana z jego digestow.
- Powod i skutek: koordynator i task maja trwaly, wspolnie weryfikowalny dowod kto, gdzie i w jakiej allowliscie moze pisac. Utrata wiadomosci, wznowienie starego CWD, timeout albo rownolegly dispatch nie tworza drugiego autora i nie wymagaja globalnego zatrzymywania pozostalych warsztatow.
- Odwolania: ARD-0108; AGENTS.md; README.md; docs/Ostatni_Pomost_architektura_Godot.txt - sekcje 2 i 13.

## ARD-0110 - Zaufany control-plane sklada asynchroniczna, shardowana bramke integracyjna GitHub

- Domena: CI, GitHub Actions, bezpieczenstwo supply chain, Git LFS, testy i integracja agentow
- Status / aktywny zakres: Obowiazuje; D1-D12
- Zatwierdzenie: 2026-08-27
- Relacje: Doprecyzowuje ARD-0108/D2,D4,D6-D12 i ARD-0109/D9-D10 | Zastapiona przez: brak
- D1. Informacja zwrotna dla autora i certyfikacja wspolnego wyniku sa dwiema bramkami. `Agent branch validation` daje szybki, read-only kontrakt galezi `codex/*`; pelna bramka integracyjna dziala asynchronicznie na immutable SHA i nie zatrzymuje authoringu, playtestu ani pracy innych warsztatow. Limit 30 sekund jest celem szybkiej informacji zwrotnej, nie obietnica pelnej regresji Godota.
- D2. Zaufany workflow z wersji domyslnej repozytorium przygotowuje plan, narzedzia walidacji i liste chronionych sciezek. Kod kandydata nie moze zastapic control-plane'u, planu shardow, agregatora ani weryfikatora podczas wlasnego przebiegu. Zwykly kandydat fail-closed odrzuca zmiane chronionego workflow, `tools/ci_*` i trzech wykonywalnych weryfikatorow Mapy; ich aktualizacja korzysta wylacznie z waskiego lane'u maintenance opisanego w D13-D14.
- D3. Pelna bramka rozdziela kanoniczny plan na cztery headless shardy i jeden shard natywny. Kazdy shard otrzymuje osobny swiezy GitHub-hosted Windows job/VM, prywatny checkout, cache, `user://`, katalog tymczasowy, porty i artefakty. Shardy moga biec rownolegle; agregacja zaczyna sie dopiero po terminalnym wyniku dokladnie wszystkich pieciu.
- D4. Plan LFS jest wyliczany centralnie dla exact base/head, ograniczony rozmiarem i liczba obiektow, a kazdy shard materializuje ten sam zweryfikowany zestaw wskaznikow. Candidate i run receipt powstaja w CI z tej samej materializacji; przychodzacy receipt autora jest wskazowka hand-offu, nie zaufanym dowodem required checku.
- D5. Plan i wyniki sa pobierane po dokladnych GitHub artifact ID, a ich nazwa, run ID, digest, waznosc i oczekiwana liczba sa sprawdzane przez API przed uzyciem. Pobieranie po samym wzorcu nazwy, przyjecie duplikatu, brak shardu, wynik obcego runu albo niezgodny digest powoduje fail-closed.
- D6. Runner tworzy w shardzie jeden immutable imported seed, a nastepnie swieza pelna kopie celu dla kazdego testu. Fingerprint wejsc musi pozostac identyczny przed i po celu; import moze dodac wylacznie natywne sidecary `.uid` i `.import`. Kazdy cel ma osobny overlay, `user://`, TEMP/TMP, porty i zaufany rekord zakonczenia.
- D7. Proces Godota otrzymuje minimalna allowliste srodowiska bez tokenow GitHub, OIDC i plikow komend workflow. Zaufany proces nadrzedny przechwytuje stdout/stderr przez pipe, rozpoczyna od stanu `FAIL`, ogranicza drzewo procesu Windows Job Object i zapisuje dokladnie jeden terminalny completion record powiazany z invocation, wyjsciem, exit code i fingerprintami wejsc.
- D8. Ta granica chroni przed kolizjami i przypadkowym albo zwyklym zapisem wspolpracujacego testu, lecz proces uruchomiony jako konto runnera nie jest sandboxem dla zlosliwego kodu. Kandydat nie otrzymuje sekretow ani uprawnien repozytorium; silniejszy model dla kodu wrogiego wymaga osobnego restricted SID/DACL albo zewnetrznego sterownika i efemerycznej guest VM.
- D9. Agregator akceptuje wylacznie dokladny zestaw pieciu receiptow shardow oraz osobny mapowy receipt z logiem, zgodne HEAD/tree/snapshot/overlay/runner/Godot/plan i terminalny `PASS` kazdego celu. Kanoniczny receipt-set wiaze surowe SHA-256 candidate receipt, agregatu Godota i mapowego receiptu. Jedynym publicznym required-check contextem jest `integration-green`, publikowanym dokladnie raz przez odseparowany GitHub App attester z tym zobowiazaniem w external ID v3; zwykly job ani kandydat nie publikuje drugiego checku o tej nazwie.
- D10. Uprawnienia sa rozdzielone: shardy sa read-only i bez sekretow, publisher ma tylko metadane oraz App token ograniczony do `checks:write`, a osobny merge krok uzywa zwyklego minimalnego `GITHUB_TOKEN`. GitHub App nie otrzymuje `Administration:write`; tozsamosc app, ruleset bez bypassu i strict wymaganie `integration-green` sa konfigurowane i audytowane przez administratora poza przebiegiem kandydata. Klucz App jest sekretem dwoch chronionych environments uzywajacych attestera, nie sekretem shardow ani repozytorium jako calosci.
- D11. `AUTO_INTEGRATOR_ENABLED` pozostaje wylaczone podczas bootstrapu. Automatyczne admission/merge wolno wlaczyc dopiero po obecnosci workflow na `main`, pierwszym zielonym exact-main audycie, skonfigurowaniu App/environment oraz rulesetu wymagajacego jednego strict `integration-green`; brak ktoregokolwiek warunku pozostawia integracje manualna i fail-closed.
- D12. Czerwony shard lub integracyjny kandydat nie przesuwa `main`, nie uniewaznia lokalnego LKG i nie blokuje galezi producentow. Autor poprawia swoja immutable rewizje nowym commitem, a integrator ponawia zlozenie na aktualnej bazie. Powodzenie publikuje wynik i receipty; nie zmienia lokalnego assignmentu ani nie czyni ruchomego worktree authority.
- D13. Rzadki lane maintenance dopuszcza w jednym PR od jednego do trzech plikow, ale wylacznie z zamknietej listy weryfikatora Mapy: builder, kontrakt portal-backdrop i smoke. Caly diff musi byc podzbiorem tej listy, bez rename; runtime, manifest, scena, generated, dokumentacja i inne payloady sa zabronione. Workflow z `main` wiaze exact PR/base/head, trzy hashe i zewnetrzny digest przegladu, uruchamia kandydata bez sekretow i bez prawa zapisu, wystawia ten sam App-owned `integration-green`, lecz nigdy nie scala. W obecnym osobistym repozytorium istnieje jeden uprawniony maintainer, dlatego environment `control-plane-maintenance` jawnie realizuje jednoosobowa reczna zgode wlasciciela z `prevent_self_review=false`; nie jest to dwuosobowa aprobata. Administracyjny bypass environment pozostaje wylaczony i jest sprawdzany podczas bootstrapu w UI, poniewaz publiczne API nie daje wystarczajacego dowodu tego ustawienia.
- D14. Certyfikowany maintenance PR jest scalany recznie. Zaufany handler `pull_request: closed` ponownie wymaga od jednego do trzech unikalnych plikow z tej samej zamknietej allowlisty, poprawnej galezi i labela, a najnowszy App-owned check tej exact rodziny `PR/base/head` musi byc zakonczonym sukcesem `control-plane-v2`; starszy sukces nie przykrywa nowszego queued, running ani failure. Handler potwierdza ancestry aktualnego `main` i dopiero wtedy wysyla idempotentny `verify-integrated-main`. Zielony exact-main audyt ustanawia nowe zdalne LKG przed zwyklym PR Mapy. Check pozostawiony jako `queued` jest bezpiecznie redispatchowany. Jawny retry po `in_progress` albo failure tworzy nowszy Check Run jako fencing generation; stary executor moze zapisac wylacznie do swojego starszego `check_run_id`, a o wyniku rodziny zawsze decyduje najwyzsze ID. Authorization i audit maja rozdzielne grupy concurrency zwiazane odpowiednio z exact zgoda oraz PR/head, wiec oczekiwanie na environment ani kolejny PR nie wypiera post-merge auditu. Awaria dispatchu konczy wybrany check porazka zamiast pozostawienia wiszacej zgody.
- Powod i skutek: ciezkie testy wykorzystuja piec rozlacznych maszyn oraz rownolegly job Mapy i przekazuja male, weryfikowalne wyniki, dzieki czemu agenci zachowuja szybka petle pracy bez wspoldzielenia Godota lub checkoutu. Cena jest odroczony pelny wynik, koszt wielu jobow, jawny bootstrap GitHub App/rulesetu, reczny jednoosobowy trust dla rzadkiej zmiany weryfikatora w obecnym repo oraz brak deklaracji sandboxu wobec zlosliwego kodu.
- Odwolania: ARD-0108; ARD-0109; AGENTS.md; README.md; docs/Ostatni_Pomost_architektura_Godot.txt - sekcje 2 i 13; `.github/workflows/agent-validation.yml`; `.github/workflows/agent-integration.yml`; `.github/workflows/agent-auto-integrator.yml`; `.github/workflows/map-control-plane-maintenance.yml`.
## ARD-0111 - Nowa kampania zastepuje poprzednia przez odzyskiwalna transakcje

- Domena: nowa kampania, persistence, powloka aplikacji, diagnostyka i odzyskiwanie po awarii
- Status / aktywny zakres: Obowiazuje; D1-D9
- Zatwierdzenie: 2026-08-27
- Relacje: Zastepuje: ARD-0092/D6 | Zastapiona przez: brak
- D1. Potwierdzone `NOWA GRA` nadal tworzy jedna biezaca kampanie w istniejacym namespace trzech snapshotow primary, pending i backup. Czwarty plik kontrolny `replace_guard` nie jest kampania, slotem ani archiwum: trwale zamyka wszystkie granice odczytu tylko na czas nierozstrzygnietego zastapienia. Nie powstaje drugi mechanizm zapisu.
- D2. `GameRoot` buduje odseparowanego kandydata, inicjalizuje cala domene, kompiluje i waliduje scene mapy oraz sprawdza profil trudnosci przed mutacja aktywnego `GameState`, sceny, narracji i flag persistence. Blad przygotowania nie dotyka aktywnego runtime ani plikow kampanii.
- D3. `SaveManager` pieczetuje jeden odseparowany surowy snapshot R nowej kampanii. Kazdy niezalezny zapis jest ponownie odczytywany bez cache, przechodzi pelna walidacje i musi zachowac metadane tozsamosci; dopiero porownanie pelnego zapisywalnego grafu ponownie odczytanych kopii wyznacza kanoniczna postac C po normalizacji serializatora. Surowego R nie porownuje sie dokladnie z plikiem `.tres`; exact dotyczy wylacznie ponownie odczytanych, zwalidowanych grafow oraz pozniejszych kopii kanonu C i nie uzywa tolerancji gameplayowej dla liczb. Transakcja nie rozpoczyna sie od usuniecia namespace i nie uznaje samego powodzenia serializacji za zatwierdzenie. Jest to jedyny jawny wyjatek od zwyklego zapisu z ARD-0092/D4.
- D4. Jezeli istnieje poprawna kampania, jej kandydat jest najpierw bezpiecznie kopiowany i ponownie walidowany jako primary bez usuwania zrodlowego kandydata. Do utworzenia `replace_guard` publiczny odczyt wybiera te kampanie; po utworzeniu guardu pozostaje ona wylacznie wewnetrznym kandydatem rollbacku, a `has_save()`, `load_game()` i zwykly zapis sa fail-closed. Ten sam surowy snapshot nowej kampanii jest niezaleznie zapisywany do pending i backup; obie kopie musza przejsc ponowny odczyt i walidacje, a ich pelne grafy musza byc dokladnie identyczne. Blad przed finalnym zatwierdzeniem przywraca poprzednia kampanie jako wynik `load_game()` dopiero po exact rollbacku i zweryfikowanym usunieciu guardu.
- D5. Przed pierwszym zapisem nowego kandydata `SaveManager` tworzy i potwierdza w tym samym katalogu trwaly `replace_guard`; porazka jego utworzenia zatrzymuje operacje przed mutacja kampanii. Finalny commit stosuje bezpieczna podmiane kanonicznego C w primary bez okna delete-first. Dla istniejacej poprawnej kampanii wynik zwykly jest sukcesem tylko wtedy, gdy wewnetrzny loader kandydatow daje exact C, a porazka `PRESERVED` tylko po przywroceniu namespace do exact old. Brak primary przy exact pending/backup=C jest odzyskiwalnym sukcesem. Niepoprawny albo obcy primary po bledzie commitu uruchamia kolejno exact C -> primary, exact old -> primary, a nastepnie zweryfikowane usuniecie primary wylacznie przy exact pending/backup=C; jezeli po tych krokach nie mozna potwierdzic exact C ani exact old, wynik jest jawnym `IN_DOUBT`, nigdy zwykla porazka obiecujaca old. Dla pierwszej kampanii to samo raw R zapisuje sie niezaleznie do primary i backup, ich cache-ignore reloady CP/CB przechodza walidacje i kontrole metadanych, a CP==CB wyznacza C. Przy rozbieznosci R jest niezaleznie zapisywane do pending i reload CT tworzy trzeciego swiadka; exact wiekszosc 2/3 sposrod CP/CB/CT wyznacza C, po czym primary i backup sa naprawiane exact do C. Brak wiekszosci albo naprawy pozwala zwrocic `PRESERVED` dopiero po potwierdzeniu przez wewnetrzny loader, ze nie istnieje poprawna kampania; nieudane wycofanie przy obserwowalnym poprawnym kandydacie daje `IN_DOUBT`, a nie falszywa porazke. Fizyczne pliki niewalidowalne moga pozostac, lecz nie wlaczaja `KONTYNUUJ`.
- D6. `KONTYNUUJ` jest dostepne tylko dla poprawnego, potwierdzonego kandydata przy braku `replace_guard`. `NOWA GRA` wymaga potwierdzenia zastapienia. Kazdy wynik zachowuje aktywny runtime bez zmian do sukcesu persistence. Zwykla porazka pokazuje konkretna przyczyne z warstwy danych, mapy albo persistence; jezeli przed proba istniala poprawna kampania, po potwierdzonym rollbacku pozostaje ona wynikiem `load_game()` i nadal zasila `KONTYNUUJ`, a bez takiej kampanii porazka nie tworzy nowej. `IN_DOUBT` nie sklada zadnej z tych obietnic: pokazuje osobny komunikat, pozostawia trwaly guard, a `has_save()`, `load_game()`, programowe Continue i zwykly autosave pozostaja fail-closed takze po restarcie. Ponowienie `NOWA GRA` moze usunac guard dopiero po zweryfikowanym commicie nowego C; nieudane ponowienie zachowuje sticky `IN_DOUBT`. Powrot do menu nadal odrzuca runtime bez zapisu i nie usuwa kampanii.
- D7. Przy braku `replace_guard` odczyt zachowuje deterministyczna kolejnosc primary -> pending -> backup; jego obecnosc zatrzymuje odczyt przed wyborem ktorejkolwiek kopii. Przy deterministycznym serializatorze Godot, bez rownoleglej zewnetrznej mutacji plikow i w fault modelu atomowego `ResourceSaver`, znane snapshoty old i C sa klasyfikowane przez dokladny ponowny odczyt; niepotwierdzony wynik nigdy nie jest przedstawiany jako `PRESERVED` lub `COMMITTED`. Guard domyka restart procesu dla niejednoznacznosci powstalej wewnatrz protokolu, ale nie potrafi uwierzytelnic pojedynczego valid-foreign snapshotu utworzonego poza nim. Kontrakt wyklucza taka zewnetrzna mutacje, nagla utrate zasilania, fizyczna awarie nosnika lub kontrolera, naruszenie trwalosci przez system operacyjny lub system plikow oraz niedeterministyczny lub bledny serializator.
- D8. Guard jest usuwany jako ostatni krok publikacji dopiero po potwierdzeniu exact C albo pelnego rollbacku do old/braku poprawnej kampanii; blad usuniecia daje `IN_DOUBT`. Sukces wymaga, aby kazdy istniejacy snapshot primary/pending/backup byl poprawny i exact C; kopia niewalidowalna albo rozbiezna blokuje publikacje nawet wtedy, gdy pierwszy kandydat loadera jest kanoniczny. Zwykly wynik ma C w primary i backup oraz brak pending. Odzyskiwalny commit z D5 moze wyjatkowo pozostawic brak primary i dwie dokladne kopie C w pending/backup. Niepowodzenie samego sprzatania pending po commicie nie odwraca wyniku tylko wtedy, gdy pozostawiony pending jest exact C. Poprzednia kampania nie pozostaje ukrytym archiwum ani awaryjnym wyborem. `SaveManager` atomowo przekazuje ponownie odczytany C wraz z wynikiem transakcji, a `GameRoot` publikuje ten kanon, nigdy raw R ani wynik osobnego, zawodnego odczytu po commicie.
- D9. Bramka testowa musi dowodzic inwariantow przygotowania kandydata przed mutacja, exact rollbacku albo exact commitu, quorum pierwszej kampanii, klasyfikacji `IN_DOUBT`, trwalosci fail-close po restarcie, kanonicznego handoffu do runtime oraz zgodnosci rzeczywistego przeplywu menu z wynikiem persistence. Szczegolowa macierz punktow awarii, kombinacji snapshotow, retry i przypadkow UI nalezy do `docs/Ostatni_Pomost_architektura_Godot.txt`, sekcja 13.
- Powod i skutek: awaria mapy, serializacji albo finalnego zapisu nie moze juz zamienic potwierdzenia `NOWA GRA` w utrate jedynego poprawnego zapisu. Dwie zwalidowane kopie nowego snapshotu przed finalna podmiana oraz rozpoznanie obserwowalnego skutku niejednoznacznego zapisu primary daja odzyskiwalny wynik dla gracza bez mnozenia kampanii, w granicach gwarancji systemu operacyjnego opisanych w D7.
- Odwolania: ARD-0001; ARD-0019; ARD-0045; ARD-0065; ARD-0092; docs/OgolnyZarys.txt - sekcja 2.1; docs/Ostatni_Pomost_architektura_Godot.txt - sekcje 11-13.
