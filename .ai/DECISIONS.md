...
- D1. Root albo koordynator przydziela jedno proste, niezalezne zadanie jednemu agentowi. Zadanie ma jeden pelny Git worktree, jedna galaz `codex/<owner>/<task-slug>` i jeden logiczny PR. Autor wykonuje `implementacja -> lokalne testy zadania -> lokalny fast-check`; po porazce poprawia zmiane, a po `PASS` wykonuje `commit -> push + PR -> KONIEC`. Nie pushuje bezposrednio do `main`, nie polluje i nie babysituje kolejki.
- D2. Lokalne testy zadania, lokalny fast-check w worktree przed commitem i wymagany GitHub `fast-check` po utworzeniu PR sa odrebnymi etapami. Fast-check moze defensywnie powtorzyc czesc testow, ale ich nie zastepuje. Tylko jawny `PASS` GitHub fast-checku dla dokladnego headu PR pozwala dodac go do merge queue; kazdy inny wynik pozostawia PR otwarty i blokuje kolejke oraz merge, takze dla rzadkiego PR control-plane. Pelna bramka `integration-green` uruchamia sie wylacznie na merge group skladajacej aktualny `main` i dany PR.
- D3. PR jest scalany metoda squash dopiero po pelnym `PASS` merge group. Merge queue sama sklada kandydat z aktualnym `main`. Prawdziwy konflikt albo wykryty przez `integration-green` nieaktualny seal, pin lub pochodna powoduje jawne zastapienie i zamkniecie starego PR przez koordynatora oraz nowe zadanie naprawcze; nowy agent startuje z aktualnego `main` w swiezym worktree, branchu i PR.
...
- D7. Automatyczna sciezka publikacji klasyfikuje workflowy, narzedzia CI, konfiguracje `integration-green` i control-plane buildera jako chronione. PR obejmujacy taki plik jest publikowany bez auto-merge i automatycznego enqueue; rzadka zmiana wymaga jawnej recznej decyzji wlasciciela i nadal przechodzi wymagany GitHub `fast-check` oraz pelny `integration-green` merge group. Jest to bramka procesu, nie twarde rozdzielenie tozsamosci GitHub; silniejsza separacja wymagalaby osobnej App lub tozsamosci i nie nalezy do tego wdrozenia.
...
- D9. Historyczne protokoly koordynacji zastapione w ARD-0108-0110 nie sa czescia zwyklej pracy autora. Root moze koordynowac wynik lub konflikt, lecz autor starego PR nie jest wznawiany jako jego staly opiekun. Szczegoly wykonania CI pozostaja wymienne, o ile zachowuja D2-D8.
...
## ARD-0114 - Warsztat Bazy jest odrebnym authority Przystani

- Domena: Baza, granice modulow, dane, assety, testy i routing pracy agentow
- Status / aktywny zakres: Obowiazuje; D1-D10, z atomowa migracja implementacji wykonywana po scaleniu prerekwizytu procesu
- Zatwierdzenie: 2026-08-29
- Relacje: Doprecyzowuje ARD-0001, ARD-0002, ARD-0004 i ARD-0113/D1-D3,D8 | Zastapiona przez: brak
- D1. `base_workbench/` jest trzecim zatwierdzonym warsztatem domenowym wewnatrz tego samego projektu Godot. Nie posiada osobnego `project.godot`, autoloadow, InputMapu, cache projektu ani lokalnego runnera; uruchomienie i testy zawsze korzystaja z projektu nadrzednego. Do czasu atomowej migracji istniejace pliki Bazy pozostaja pod dawnymi sciezkami Rootu i samo wydzielenie prowadzi owner `integration`.
- D2. Po atomowej migracji warsztat Bazy jest jedynym wlascicielem aktywnej `BaseScene.tscn`, `BaseController`, lokalnego srodowiska 2D/3D, UI zarzadzania Przystania, systemow budowy, obsady, pracy, produkcji, racji, opieki i wydarzen osady oraz ich definicji, danych, assetow, narzedzi authoringu i testow domenowych. Root nie utrzymuje kopii pod dawnymi sciezkami.
- D3. Root pozostaje jedynym wlascicielem `GameRoot`, `GameState`, persistence, `EndOfDayResolver`, kampanii i misji, wspolnych systemow kariery, kompetencji, talentow, chorob, urazow i rotacji mieszkancow, pogody i trudnosci oraz przygotowania wyprawy i wyposazenia nurkowego. Wydzielenie plikow nie tworzy drugiego agregatu ani alternatywnej transakcji dnia.
- D4. Publiczna granica Root-Baza sklada sie z bezposrednio montowanej `base_workbench/runtime/BaseScene.tscn`, jawnych metod i sygnalow kontrolera, lokalnych systemow operujacych na publicznych typach Rootu oraz walidowanych definicji i danych ladowanych przez `GameDatabase`. Zaleznosc w obu kierunkach nie upowaznia do kopiowania prywatnej sceny, danych albo stanu.
- D5. Definicje budynkow, receptur i wydarzen oraz ich zasoby sa lokalnym, zamknietym katalogiem warsztatu. Relokacja zachowuje stabilne ID, klasy zasobow, wartosci, kolejnosc selekcji i semantyke istniejacego formatu kampanii; zmiana znaczenia danych nadal wymaga zwyklej bramki produktu, architektury i zapisu.
- D6. Lokalny test systemu, sceny, UI, grafiki, modelu, muzyki lub capture Bazy nalezy do `base_workbench/tests/` i korzysta ze wspolnego izolowanego runnera. Test atomowosci zapisu, pelnej kolejnosci dnia albo kompozycji wielu ownerow pozostaje w Root. Bezposredni cel Bazy nie wykonuje discovery Mapy, Nurka ani pakietow struktur.
- D7. Pierwsze wydzielenie jest relokacja bez zmiany zachowania widocznego dla gracza, balansu, publicznego API, kolejnosci resolvera, formatu kampanii, migracji, wartosci `Resource` ani parametrow importu Ogg i GLB. Dawne `scenes/base`, `scripts/base`, bazowe katalogi `assets` i danych staja sie po migracji zabronionymi kopiami, nie aliasami kompatybilnosciowymi.
- D8. Maszynowy kontrakt write-set rozpoznaje ownera `base` dla `base_workbench/**` obok `root`, `map`, `diver`, `structure:<id>` i `integration`. Zwykle zadanie jednej domeny zapisuje tylko jej katalog w osobnym pelnym worktree i na `codex/<owner>/<task-slug>`; zadanie przekrojowe uzywa `integration`. Lokalny oraz GitHubowy fast-check odrzucaja przekroczenie ownera, a publisher przed pushem zatrzymuje konflikt z aktualnym `origin/main`. Dalsze przyjecie i squash do `main` pozostaja odpowiedzialnoscia wymaganych checkow i merge queue zgodnie z ARD-0113.
- D9. Warsztat posiada dokladnie lokalne `AGENTS.md`, `README.md`, `.ai/PROJECT_CONTEXT.md` i `.ai/DECISIONS.md` z rejestrem `BASE-ARD`. Nie przejmuje globalnych regul gry, persistence ani historii decyzji Rootu i nie tworzy dodatkowej dokumentacji kontraktowej.
- D10. Zachowany `BuildingBlueprintSystem` bez producenta i konsumenta pozostaje jawnie nieaktywny. Umieszczenie go w `systems/inactive/` nie aktywuje mechaniki, nie ustanawia API i nie jest podstawa do wdrozenia nowej funkcji bez odrebnego rozstrzygniecia.
- Powod i skutek: agent Bazy otrzymuje jeden kompletny, uporzadkowany pakiet scen, mechanik, logiki, grafiki i danych Przystani, a przekrojowy stan i transakcje pozostaja w Root. Rozlaczne katalogi i egzekwowany owner pozwalaja wielu agentom pracowac rownolegle bez wspolnego zapisywalnego checkoutu; konflikt tego samego pliku nie jest ukrywany i wymaga nowego zadania z aktualnego `main`.
- Odwolania: AGENTS.md; docs/Ostatni_Pomost_architektura_Godot.txt - sekcje 2, 4, 6, 12.2 i 13; przyszle `base_workbench/.ai/DECISIONS.md` po atomowej migracji.
