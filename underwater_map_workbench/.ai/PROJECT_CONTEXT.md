...
- `map_manifest.json`, `UnderwaterMap.tscn`, mapowe metadane builda i `structures/*/generated/**` zmienia wyłącznie zwykły PR właściciela Mapy. Przy kilku nowych sealed manifestach builder może odświeżyć wszystkie dokładne SHA-256 jednym batchem i musi pozostawić spójny zestaw pochodnych.
- Builder i runner odrzucają niezgodne EOL oraz izolują `.godot`, `user://`, logi i porty. Pełne testy złożenia w merge queue przed scaleniem są zatwierdzonym celem ARD-0111; bieżący stan wdrożenia opisuje globalny `.ai/PROJECT_CONTEXT.md`.
