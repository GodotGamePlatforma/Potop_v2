...
- W01 jest przypięty do sealed manifestu `b7d382f2…ad844`. Produkcyjna L04 została zbudowana od pustego natywnego rastra `2240 × 3680` jako wyłącznie open-water, bez użycia poprzedniego RGB i bez resize'u lub resamplingu; alfa nadal odpowiada kanonicznej masce piksel w piksel. Mapowa `presentation_revision` wynosi `center-station-right-city-two-tower-production-v20`, a lokalny digest topologii struktury, globalna topologia, partycja ownerów i sygnatura gameplayu pozostają bez zmian.
...
- 2026-08-29: W01 przyjął sealed manifest `b7d382f2…ad844`; celowany build/check pakietu oraz pełny mapowy `--build` i niedestrukcyjny `--check` odtworzyły presentation v20 (`presentation-v2:dc75453c…88865`) bez zmiany lokalnej lub globalnej topologii, partycji ownerów ani sygnatury gameplayu.
- Portal contract przeszedł `6/6`. Kontrakt i runtime W01, map smoke, boundary, publiczne osadzenie struktury oraz clearance Nurka 105 × 60 przeszły po `1/1 PASS`, bez `ERROR` i `SCRIPT ERROR`. Kontrakt bitmap potwierdził `2240 × 3680`, `scale=Vector2.ONE`, binarną alfę, `mismatch=0`, `overlap=0`, `gap=0` i brak RGB pod alfą zero.
- Natywny capture W01 oraz świeży survey prawdziwego `UnderwaterMap.tscn` przeszły po `1/1 PASS`. Ręczne oględziny L04-only, L05-only, composite, overlay collidera, entry, B/C/D, szybu, piwnicy, drzwi CLOSED/MID/OPEN i pełnego sweepu wózka zakończyły się `ART PASS`: w open-water nie ma pozornych przeszkód, beton występuje wyłącznie na solid, a zachodni ciemny prostokąt przy entry zniknął w obu kierunkach bez halo i bez regresji z-orderu lub paralaksy.
