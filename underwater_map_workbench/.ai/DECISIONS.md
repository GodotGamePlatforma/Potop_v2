...
- D6. Autor uruchamia wymagany seal/refresh/build/check, celowane testy oraz lokalny fast-check w worktree przed commitem. Następnie tworzy PR, włącza GitHub `merge when ready` metodą squash i kończy pracę bez pollingu. Osobny wymagany GitHub `fast-check` sprawdza dokładny head i tylko jego `PASS` pozwala enqueue; pełną integrację na merge group `aktualny main + PR` chroni następnie `integration-green` zgodnie z ARD-0113.
- D7. `structures/<id>/generated/**`, generowana scena, L05 i mapowe metadane są deterministycznymi pochodnymi. Nie wolno poprawiać ich ręcznie ani używać ich dryfu jako drugiego źródła prywatnego stanu.
