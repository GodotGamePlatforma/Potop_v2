...
- Każdy równoległy producent Mapy, Nurka i `structures/<id>/` pracuje w osobnym pełnym Git worktree i na gałęzi `codex/<owner-segment>/<task-slug>` z commita integracyjnego potwierdzonego candidate receiptem oraz zgodnym pełnym run receiptem `PASS`. Oficjalny helper zachowuje nazwy ownerów `map` i `diver`, a `structure:<id>` mapuje na segment `structure-<id>`, ponieważ dwukropek nie jest dozwolony w refie Git. Rozłączna allowlista nadal jest obowiązkowa; osobny CWD we wspólnym worktree nie wystarcza.
- Builder i wspólny runner przed pracą uruchamiają repozytoryjną bramkę EOL: tracked plik z `eol=lf` w stanie `w/crlf` albo `w/mixed` jest błędem. Legalny dirty authoring pozostaje dozwolony, jeśli jego surowe bajty używają LF; candidate receipt dodatkowo wymaga zgodności surowych bajtów czystego drzewa z exact HEAD/index.
