...
- Zadania zapisujące wspólny zestaw publikacji Mapy — `map_manifest.json`, `UnderwaterMap.tscn`, mapowe metadane builda lub `structures/*/generated/**` — root lub koordynator uruchamia i publikuje sekwencyjnie z aktualnego `origin/main`; kolejne takie zadanie zaczyna się dopiero po stanie `MERGED` albo `CLOSED` poprzedniego PR, a inne zadania o rozłącznych zapisach nadal pracują równolegle.
