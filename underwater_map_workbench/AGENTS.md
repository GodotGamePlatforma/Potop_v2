...
- Każdy równoległy producent Mapy, Nurka i `structures/<id>/` pracuje w osobnym pełnym Git worktree z aktualnego `origin/main` i na gałęzi `codex/<owner-segment>/<task-slug>`. Oficjalny helper zachowuje nazwy ownerów `map` i `diver`, a `structure:<id>` mapuje na segment `structure-<id>`, ponieważ dwukropek nie jest dozwolony w refie Git. Setup nie wymaga receiptu ani LKG, lecz tworzy trwały assignment z zamkniętą allowlistą; zapis zaczyna się dopiero po ACK i stanie `RUNNING`. Zielony PR przechodzi automatyczny `fast-green`, dla wrażliwego diffu także zaufany Codex gate, natywną kolejkę GitHub i squash merge. Osobny CWD we wspólnym worktree nie wystarcza.
- Builder i wspólny runner przed pracą uruchamiają repozytoryjną bramkę EOL: tracked plik z `eol=lf` w stanie `w/crlf` albo `w/mixed` jest błędem. Legalny dirty authoring pozostaje dozwolony, jeśli jego surowe bajty używają LF. Receipty tworzone wewnętrznie przez runner albo integratora są diagnostyką i nie wchodzą do zwykłej publikacji PR.
