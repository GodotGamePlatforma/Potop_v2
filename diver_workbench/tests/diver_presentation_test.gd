...
const APPROVED_SOURCE_UNION := Rect2(-207, -82, 420, 199)
const APPROVED_WORLD_ALPHA_SIZE := Vector2(100.38, 47.561)
...
const MAXIMUM_LOCOMOTION_GUARD_OFFSET := 2.0
...
func _test_frame_envelope_contract() -> void:
	...
	for animation_name: StringName in ANIMATION_SOURCES:
		...
		for frame in range(16):
			...
			for fin_socket: StringName in [&"fin_upper", &"fin_lower"]:
				...
				_check(_has_alpha_near(frame_image, authored_fin, 4, 16), "%s/%s frame %d should remain attached to visible fin geometry." % [animation_name, fin_socket, frame])
	...
	_check(measured_union == APPROVED_SOURCE_UNION, "The measured 48-frame source union should match the independently approved 420 x 199 raster authority.")
	_check(DiverFrameEnvelopeScript.SOURCE_UNION == APPROVED_SOURCE_UNION, "Runtime alpha-envelope data should publish the independently approved source union.")
	...
	_check(base_world_size.is_equal_approx(APPROVED_WORLD_ALPHA_SIZE), "The authored alpha union should retarget to 100.38 x 47.561 world units.")
	...
	_test_stable_body_registration(source_images)
	_test_raster_kick_contract(source_images)


func _test_stable_body_registration(source_images: Dictionary) -> void:
	var idle_sheet := source_images.get(&"idle") as Image
	_check(idle_sheet != null, "Idle sheet should be available as the stable-body reference.")
	if idle_sheet == null:
		return
	var reference_frame := idle_sheet.get_region(Rect2i(Vector2i.ZERO, DiverFrameEnvelopeScript.FRAME_SIZE))
	var reference_bytes := reference_frame.get_region(STABLE_BODY_REGION).get_data()
	for animation_name: StringName in ANIMATION_SOURCES:
		...
		for frame in range(16):
			...
			var registered_bytes := frame_image.get_region(STABLE_BODY_REGION).get_data()
			_check(registered_bytes == reference_bytes, "%s frame %d should keep the helmet, tank and upper body pixel-registered to the shared stable raster." % [animation_name, frame])
...
func _alpha_components_in_region(
	image: Image,
	region: Rect2i,
	minimum_alpha: int,
	minimum_area: int
	...
	var region_width: int = region.size.x
	var region_height: int = region.size.y
	var visited := PackedByteArray()
	visited.resize(region_width * region_height)
	var queue := PackedInt32Array()
	var components: Array[Dictionary] = []
	for start in range(region_width * region_height):
		if visited[start] != 0:
			continue
		var local_x: int = start % region_width
		var local_y: int = start / region_width
		...
		while head < queue.size():
			var current: int = queue[head]
			head += 1
			var x: int = current % region_width
			var y: int = current / region_width
			area += 1
			...
			if x + 1 < region_width:
				_region_alpha_enqueue(current + 1, region, image_width, minimum_alpha, pixels, visited, queue)
			if y > 0:
				_region_alpha_enqueue(current - region_width, region, image_width, minimum_alpha, pixels, visited, queue)
...
func _region_alpha_enqueue(
	...
	var local_x := local_index % region.size.x
	var local_y := local_index / region.size.x
	var image_index := (region.position.y + local_y) * image_width + region.position.x + local_x
	if int(pixels[image_index * 4 + 3]) >= minimum_alpha:
		queue.append(local_index)


func _test_raster_kick_contract(source_images: Dictionary) -> void:
...
func _nearest_component_index(components: Array[Dictionary], point: Vector2) -> int:
	...
	for index in range(components.size()):
		var centroid: Vector2 = components[index]["centroid"]
		var distance := centroid.distance_squared_to(point)
...
func _test_animation_timing_contract() -> void:
	...
	for animation_name: StringName in [&"swim", &"sprint"]:
		...
		_check(minimum_separation >= 10.0, "%s anatomical near/far fin attachment tracks must remain separated in every frame." % animation_name)
		_check(maximum_separation - minimum_separation >= 20.0, "%s anatomical fin attachment spacing must vary across the cycle." % animation_name)
		_check(upper[4].y - lower[4].y <= -70.0, "%s near-leg attachment should be above the far leg at the first kick extreme." % animation_name)
		_check(upper[12].y - lower[12].y >= 55.0, "%s near-leg attachment should be below the far leg at the opposite kick extreme." % animation_name)
...
func _test_pre_ready_quality_bridge() -> void:
	...
	_check(EnvelopeProfile.authored_sprite_position.is_equal_approx(Vector2(-0.717, -4.1825)), "The active profile should own the reviewed raster centering before ready.")
...
func _test_runtime_presentation_contract() -> void:
	...
	_check(sprite.scale.is_equal_approx(EnvelopeProfile.authored_sprite_scale), "Runtime ready should apply the active authored scale to only the visual branch.")
	_check(sprite.position.is_equal_approx(EnvelopeProfile.authored_sprite_position), "Runtime ready should center the right-facing alpha union from the active profile.")
