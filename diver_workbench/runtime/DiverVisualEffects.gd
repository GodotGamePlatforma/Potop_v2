...
func _ready() -> void:
	...
	_visual_retarget_scale = (
		minf(absf(diver_sprite.scale.x), absf(diver_sprite.scale.y))
		/ PREVIOUS_AUTHORED_SPRITE_SCALE
	)
...
func _create_leak_material() -> ParticleProcessMaterial:
	...
	material.scale_max = 0.58 * _visual_retarget_scale
	material.emission_shape = ParticleProcessMaterial.EMISSION_SHAPE_SPHERE
	material.emission_sphere_radius = 1.8 * _visual_retarget_scale
